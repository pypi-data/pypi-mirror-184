    ...
    def manualtransitions(self):
        try:
            ...
            obj , created  = workflowitems.objects.update_or_create( transitionmanager= queryset , defaults= {"initial_state" : self.source, "interim_state" : self.interim ,
                    "final_state" :  self.target  , "next_available_transitions" : None, "record_datas" : self.get_record_datas(),"model_type" : self.type, "event_user" : get_current_user() , "current_from_party" : self.from_party, "current_to_party" : self.to_party ,"final_value" : True})
