...
def printTable(list, info):
    error_table = PrettyTable("编号", "组件名", "分支", "错误信息")
    for index in range(0, len(list) - 1):
        model = list[index]
        error_table.add_row(index, model.module, model.branch, model.msg)
    error_table.align[1] = '1'
    error_table.border = True
    error_table.junction_char = '+'
    error_table.horizontal_char = '——'
    error_table.vertical_char = '|'
    debugInfo(error_table)
    debugInfo("[!] 以上是{0}".format(info))
...
class yhgit:
    ...
    def status(self):
        ...
        if len(status_result) == 0:
            ...
            printTable(error_module, "检查失败的模块")
            # logging.info(fail_module)
            # logging.info(succ_module)
