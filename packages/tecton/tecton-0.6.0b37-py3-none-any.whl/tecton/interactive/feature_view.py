...
class FeatureView(FeatureDefinition):
    ...
    def get_historical_features(
        ...
        if not from_source and is_dev_workspace and not is_snowflake_compute:
        ...
        if not from_source and not feature_definition.writes_to_offline_store and not is_snowflake_compute:
            # Snowflake compute always computes "from_source" and ignores the from_source parameter.
...
class OnDemandFeatureView(FeatureView):
    ...
    def get_historical_features(
        ...
        if (
            is_dev_workspace
            and not from_source
            and (feature_table_inputs or feature_view_inputs)
            and not is_snowflake_compute
        ):
        ...
        for feature_definition_input in feature_view_inputs + feature_table_inputs:
            if not feature_definition_input.writes_to_offline_store and not from_source and not is_snowflake_compute:
                # Snowflake compute always computes "from_source" and ignores the from_source parameter.
