...
# Note the "firstn" here represents the first non-distinct n aggregation, which does not follow the pattern created by "lastn" above which represents the last distinct n aggregation.
def first(n: int) -> AggregationFunction:
    return AggregationFunction("firstn", {"n": n})
