...
def test_cp_1d_pde():
    ...
    bcs = [
        (
            NeumannBoundaryCondition(
                lambda x, t: np.zeros((1, 1)), is_static=True
            ),
        )
        * 2
    ]
    ...
    assert np.all(d_y_vertex_boundary_constraints[0, 0][0].values == [0.0])
    assert np.all(d_y_vertex_boundary_constraints[0, 0][1].mask == [True])
    assert np.all(d_y_vertex_boundary_constraints[0, 0][1].values == [0.0])
    ...
    assert np.all(d_y_cell_boundary_constraints[0, 0][0].values == [0.0])
    assert np.all(d_y_cell_boundary_constraints[0, 0][1].mask == [True])
    assert np.all(d_y_cell_boundary_constraints[0, 0][1].values == [0.0])


def test_cp_2d_pde():
    diff_eq = WaveEquation(2)
    mesh = Mesh([(2.0, 6.0), (-3.0, 3.0)], [0.1, 0.2])
    bcs = (
        (
            DirichletBoundaryCondition(
                vectorize_bc_function(lambda x, t: (999.0, None)),
                is_static=True,
            ),
            NeumannBoundaryCondition(
                vectorize_bc_function(lambda x, t: (100.0, -100.0)),
                is_static=True,
            ),
        ),
        (
    ...
    y_vertices = np.full(cp.y_shape(True), 13.0)
    apply_constraints_along_last_axis(
        cp.static_y_vertex_constraints, y_vertices
    )

    assert np.all(y_vertices[0, :-1, 0] == 999.0)
    assert np.all(y_vertices[0, :-1, 1] == 13.0)
    assert np.all(y_vertices[-1, :-1, :] == 13.0)
    assert np.all(y_vertices[1:, 0, :] == 13.0)
    assert np.allclose(
        y_vertices[:, -1, 0], np.linspace(2.0, 6.0, y_vertices.shape[0])
    )
    assert np.all(y_vertices[:, -1, 1] == 3.0)
    ...
    assert np.all(d_y_0_over_d_x_0[-1, :, :] == 100.0)
    assert np.all(d_y_0_over_d_x_0[:-1, :, :] == 0.0)
    ...
    assert np.allclose(
        d_y_0_over_d_x_1[:, 0, 0], np.linspace(-2.0, -6.0, y_vertices.shape[0])
    )
    assert np.all(d_y_0_over_d_x_1[:, 1:, :] == 0.0)

    d_y_1_over_d_x_0 = diff.gradient(
        y_vertices[..., 1:], mesh, 0, d_y_boundary_constraints[:, 1:]
    )

    assert np.all(d_y_1_over_d_x_0[-1, :, :] == -100.0)
    assert np.all(d_y_1_over_d_x_0[:-1, :, :] == 0.0)

    d_y_1_over_d_x_1 = diff.gradient(
        y_vertices[..., 1:], mesh, 1, d_y_boundary_constraints[:, 1:]
    )

    assert np.all(d_y_1_over_d_x_1 == 0.0)
    ...
    assert np.all(y_boundary_cell_constraints[0, 0][0].values == 999.0)
    assert np.all(
    ...
    assert np.all(y_boundary_cell_constraints[1, 1][1].values == 3.0)
    assert y_boundary_cell_constraints[1, 0][0] is None


def test_cp_3d_pde():
    mesh = Mesh([(2.0, 6.0), (-3.0, 3.0), (10.0, 12.0)], [0.1, 0.2, 0.5])
    ...
    cp = ConstrainedProblem(
        ...
        (
            ...
            (
                NeumannBoundaryCondition(
                    lambda x, t: -x[:, :2] * x[:, 1:3], is_static=True
                ),
                DirichletBoundaryCondition(
                    vectorize_bc_function(lambda x, t: (-999.0, None))
                ),
    ...
    assert np.all(y[0, 1:, :, 0] == 999.0)
    assert np.all(y[:, 0, :, 0] == 0.0)
    assert np.all(y[1:, 1:, :, 0] == -1.0)
    assert np.all(y[:, 0, :, 1] == 0.0)
    assert np.all(y[:, 1:, :, 1] == -1.0)
    ...
    for y_boundary_constraints in [
        vertex_boundary_constraints[0],
        cell_boundary_constraints[0],
    ]:
    ...
    for d_y_boundary_constraints in [
        vertex_boundary_constraints[1],
        cell_boundary_constraints[1],
    ]:
    ...
    new_vertex_boundary_constraints = cp.create_boundary_constraints(True, 1.0)
    ...
    assert np.all(d_y_boundary == 1.0)

    new_y_vertex_constraints = cp.create_y_vertex_constraints(
        new_y_boundary_constraints
    )
    ...
    assert np.all(y[0, 1:, :-1, 0] == 999.0)
    assert np.all(y[:, 0, :-1, 0] == 0.0)
    assert np.all(y[:, :, -1, 0] == -999.0)
    assert np.all(y[1:, 1:, :-1, 0] == -1.0)
    assert np.all(y[:, 0, :, 1] == 0.0)
    assert np.all(y[:, 1:, :, 1] == -1.0)
...
def test_cp_pde_with_wrong_boundary_constraint_length():
    ...
    mesh = Mesh([(0.0, 5.0), (-5.0, 5.0)], [0.1, 0.2])
    static_bcs = [
        (
            DirichletBoundaryCondition(
                lambda x, t: np.zeros((13, 1)), is_static=True
            ),
        )
        * 2
    ] * 2
    ...
    with pytest.raises(ValueError):
        cp.create_boundary_constraints(True, 0.0)
