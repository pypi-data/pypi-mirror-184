...
matching_time_points = np.linspace(
    t_lower + (t_upper - t_lower) / 4.0, t_upper, 4
)
...
def plot_coarse_solution_diffs(ar_model_ind: int, pidon_model_ind: int):
    coarse_ar.model.model.load_weights(
        f"weights/ar_{ar_model_ind}"
    ).expect_partial()
    coarse_pidon.model.load_weights(
        f"weights/pidon_{pidon_model_ind}"
    ).expect_partial()
    ...
    diff = fine_fdm_sol.diff([coarse_fdm_sol, coarse_ar_sol, coarse_pidon_sol])
...
def plot_rss_solution_diffs(
    rss_diffs: Sequence[Sequence[float]], labels: Sequence[str], name: str
):
    fig, ax = plt.subplots()
...
if __name__ == "__main__":
            ...
            if match:
                ...
                if model_type == "ar":
                    ...
                    test_loss = sum(
                        [
                            float(loss)
                            for loss in match.group(3)[1:-1].split(" ")
                        ]
                    )
    ...
    for rank, times_by_label in enumerate(aggregate_exec_times):
        plot_execution_times(
            *zip(*times_by_label.values()),
            list(times_by_label.keys()),
            "operator",
    ...
    for n_iterations_minus_one, rss_solution_diffs_for_iterations in enumerate(
        rss_solution_diffs[:-1]
    ):
        for rank, rss_solution_diffs_for_rank_and_iterations in enumerate(
            rss_solution_diffs_for_iterations
        ):
            max_rss_diff = np.max(
