...
class Seda:
    ...
    def task(self, *args: t.Any, **kwargs: t.Any) -> t.Callable:
        ...
        def decorator(f: t.Callable) -> t.Callable:
            ...
            wrapper.at = lambda t, **options: (  # type: ignore[attr-defined]
                functools.partial(
                    self.schedule,
                    expression=f"at({t.replace(microsecond=0).isoformat()})",
                    onetime=f,
                )(**options)
            )
    ...
    def schedule(
        ...
        *,
        ...
        onetime: t.Optional[t.Callable] = None,
    ) -> t.Union[t.Callable, str]:
        ...
        if onetime is not None:
            return self.create_schedule(partial(onetime), onetime=True)
    ...
    def get_function(self) -> t.Dict[str, t.Any]:
    ...
    def create_schedule_role(self) -> str:
    ...
    def delete_schedule_role(self) -> None:
        role_name = self.config.get_schedule_role_name()
        return self.client.delete_role(role_name)

    def put_function_policy(self) -> None:
        ...
        self.client.put_role_policy(function_role_name, policy_name, policy)

    def delete_function_policy(self) -> None:
        function_role_name = self.get_function_role_name()
        policy_name = self.config.get_function_policy_name()
        self.client.delete_role_policy(function_role_name, policy_name)

    def get_schedule_group(self, onetime: bool = False) -> t.Dict[str, t.Any]:
        group_name = self.config.get_schedule_group_name(onetime=onetime)
        return self.client.get_schedule_group(group_name)

    def create_schedule_group(self, onetime: bool = False) -> str:
        group_name = self.config.get_schedule_group_name(onetime=onetime)
        while True:
            try:
                return self.client.create_schedule_group(group_name)
            except exceptions.AlreadyExistsError:
                if self.get_schedule_group()["State"] == "DELETING":
                    time.sleep(DEFAULT_RETRY_DELAY)
                    continue
                raise

    def delete_schedule_group(self, onetime: bool = False) -> None:
        group_name = self.config.get_schedule_group_name(onetime=onetime)
        self.client.delete_schedule_group(group_name)
    ...
    def create_schedule(self, schedule: Schedule, onetime: bool = False) -> str:
        role_name = self.config.get_schedule_role_name()
    ...
    def delete_sns_topic(self) -> None:
        ...
        self.client.delete_sns_topic(topic_arn)

    def add_sns_permission(self) -> None:
        topic_arn = self.ARN(f"sns:{self.config.get_sns_topic_name()}")
        self.client.add_lambda_permission(
    ...
    def remove_sns_permission(self) -> None:
        self.client.remove_lambda_permission(
            function_name=self.config.function_name,
            statement_id=self.config.get_sns_statement_id(),
        )

    def sns_subscribe(self) -> str:
        topic_arn = self.ARN(f"sns:{self.config.get_sns_topic_name()}")
        function_arn = self.ARN(f"lambda:function:{self.config.function_name}")
