...
# }
torch_credentials = {
    'url': 'https://acceldata.dev.10.90.3.89.nip.io/torch',
...
torch_client = TorchClient(**torch_credentials)'''
'''
asset = torch_client.get_asset('sf_ds.FINANCE.FINANCE.ACCELDATA_CUSTOMERS_IND')
pp.pprint('asset')
pp.pprint(asset)
ds_name = torch_client.get_datasource('sf_ds', True)
...
# pp.pprint(dq_rule_executions)
# dq_rule_executions = torch_client.policy_executions('test_dq_age_null', RuleType.DATA_QUALITY)
# pp.pprint('dq_rule_executions')
# pp.pprint(dq_rule_executions)
...
# pp.pprint('dq_rule_executions')
# pp.pprint(dq_rule_executions)
# # async_executor = torch_client.execute_policy(const.PolicyType.DATA_QUALITY, dq_policy_id, sync=False)
...
dq_policy_id = 3
torch_client.get_policy(const.PolicyType.RECONCILIATION, dq_policy_id)
dq_rule_executions = torch_client.policy_executions(dq_policy_id, RuleType.RECONCILIATION)
pp.pprint('dq_rule_executions')
pp.pprint(dq_rule_executions)
dq_rule_executions = torch_client.policy_executions('recon_customer', RuleType.RECONCILIATION)
pp.pprint('dq_rule_executions')
pp.pprint(dq_rule_executions)
filter = PolicyFilter(policyType=RuleType.RECONCILIATION, enable=True)
dq_rules = torch_client.list_all_policies(filter=filter)
pp.pprint(dq_rules)
dq_rule = torch_client.get_policy(const.PolicyType.RECONCILIATION, dq_policy_id)
pp.pprint('dq_rule')
pp.pprint(dq_rule)
dq_rule_executions = dq_rule.get_executions()
pp.pprint('dq_rule_executions')
