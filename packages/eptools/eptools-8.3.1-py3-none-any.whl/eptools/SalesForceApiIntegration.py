...
def getHubAndRound(client_nr):
    try: 
        db_conn = pyodbc.connect(driver = '{SQL Server Native Client 11.0}',
                                       ...
                                       user = 'sa',
                                       password = 'sql')
        sql = """SELECT inr.CustomerIdPost,inr.CustomerParentIdPost,inr.Hub as Hub, outr.Hub as ParentHub, inr.RouteName as RouteName, outr.RouteName as ParentRouteName
  FROM [EasyPost].[dbo].[SFAccounts] inr
  left join [EasyPost].[dbo].[SFAccounts] outr  on inr.CustomerParentIdPost = outr.CustomerIdPost and inr.CustomerIdPost != 0 and outr.CustomerIdPost != 0
