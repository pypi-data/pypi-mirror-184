...
def _get_roles_from_cloud_resource(
    cloud_resource: CreateCloudResource, boto3_session: boto3.Session
) -> List[Any]:
...
def verify_aws_iam_roles(
    ...
    if not cloud_resource.aws_iam_role_arns:
        ...
        return False
    roles = _get_roles_from_cloud_resource(cloud_resource, boto3_session)
...
def verify_aws_efs(
    ...
    if not file_systems_response.get("FileSystems"):
