...
class PostgreSQLSourceImpl(BaseSQLSource, TableIngestMixIn, SQLSourceWithTableIngestProtocol):
    def __init__(
        ...
        try:
            ...
            raise ChalkConfigurationException.missing_dependency("chalkpy[postgresql]")
        del psycopg2  # unused
    ...
    def execute_query(self, finalized_query: FinalizedChalkQuery, connection: Optional[Connection] = None) -> pa.Table:
        import psycopg2.sql

        if env_var_bool("CHALK_DISABLE_POSTGRES_CSV_TO_PYARROW"):
            return super().execute_query(finalized_query, connection)
