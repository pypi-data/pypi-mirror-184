...
from iacer import cli_modules
from iacer.cli import CliCore, GLOBAL_ARGS, _get_log_level
from iacer.config import IAC_OCTOPUS_NAME
from iacer.logger import init_cli_logger
from iacer.util import exit_with_code
...
async def run():
    ...
    try:
        ...
        cli = CliCore(IAC_OCTOPUS_NAME, cli_modules, DESCRIPTION, version, GLOBAL_ARGS.ARGS)
        cli.parse(args)
...
def get_installed_version():
    try:
        return get_distribution(IAC_OCTOPUS_NAME).version
    except Exception:
