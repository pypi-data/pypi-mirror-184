...
class ParamGenerator:
    RE_V_AUTO = re.compile(fr'\$\[{IAC_OCTOPUS_NAME}-auto]', re.I)
    RE_V_CURRENT_REGION = re.compile(fr'\$\[{IAC_OCTOPUS_NAME}-current[-_]region]', re.I)

    RE_K_ZONE_ID = re.compile('zone(_|)id', re.I)
    RE_K_VPC_ID = re.compile('vpc(_|)id', re.I)
    RE_K_VSW_ID = re.compile('v(_|)switch(_|)id', re.I)
    RE_K_SECURITY_GROUP = re.compile('security(_|)group(_id|id)', re.I)
    RE_K_COMMON_NAME = re.compile(r'\w*name', re.I)
    RE_K_PASSWORD = re.compile(r'\w*password', re.I)
    RE_K_UUID = re.compile(r'\w*uuid', re.I)
    ...
    async def resolve_auto_key(self):
        for key, unresolved_value in self._unresolved_parameters.items():
            if not isinstance(unresolved_value, str) or not self.RE_V_AUTO.match(unresolved_value):
                continue

            if self.RE_K_VSW_ID.match(key):
    ...
    async def _gen_vpc_vsw_id(self, key, value):
        ...
        for name, value in self.parameters.items():
            if self.RE_K_ZONE_ID.match(name):
                zone_id = value
                break
