...
def des_encrypt(message, key):
    ''' key: 8位 '''
    message = _pad(to_bytes(message), DES.block_size)
    key = to_bytes(key)
    cipher = DES.new(to_bytes(key), DES.MODE_CBC, key)
...
def aes_encrypt(message, key, iv=None):
    ''' key: 32位, iv: 16位 '''
    message = _pad(to_bytes(message), AES.block_size)
    key = to_bytes(key)
    iv = iv or key[:16]
