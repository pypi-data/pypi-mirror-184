...
class QAT(Magics):
    ...
    def plot(self, circuit, depth, filename=None):
        ...
        try:
            ...
            tmp_pdf_file = (
                tmpf.NamedTemporaryFile(prefix="qat2pdf_", suffix="_circ.pdf", dir=".") if filename is None else open(filename, "w")
            )
            ...
            with tmp_pdf_file, tmpf.NamedTemporaryFile(prefix="qat2pdf_", suffix="_circ.png", dir=".") as tmp_png_image:
                ...
                # Create PNG file
                pdf2png(
                    pdf_file=tmp_pdf_filename_radix + ".pdf",
                    png_file=tmp_png_image.name,
                )

                # Display image
                self.shell.run_cell(
                    f"import IPython; IPython.display.Image({tmp_png_image.name!r})",
                    store_history=False,
                )
