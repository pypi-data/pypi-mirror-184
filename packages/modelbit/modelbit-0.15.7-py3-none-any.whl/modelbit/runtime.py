...
from .utils import unindent, isMainModuleFunction
...
class Runtime:
    ...
    for maybeFuncVarName in allNames:
      if maybeFuncVarName in globalsDict:
        ...
        if "__module__" in dir(maybeFuncVar):
          if isMainModuleFunction(maybeFuncVar):  # the user's functions
            self._collectFunction(maybeFuncVar, collection)
  ...
  def _collectFunction(self, func: Callable[..., Any], collection: NamespaceCollection):
    ...
    if funcSig not in collection.functions:
      ...
      self._collectNamespaceDeps(func, collection)
