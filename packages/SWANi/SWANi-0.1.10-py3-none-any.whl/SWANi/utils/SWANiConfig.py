...
class SWANiConfig(configparser.ConfigParser):
    ...
    def savePatientConfig(self):
        with open(self.patientConfigFile,"w") as openedFile:
            self.patientConfiguration.write(openedFile)
    
    def getPatientWfType(self):
        self.patientConfigFile=os.path.join(os.getcwd(),".config")
        if os.path.exists(self.patientConfigFile):
            self.patientConfiguration.read(self.patientConfigFile)
        else:
            self.patientConfiguration['ptConfig']={'wfType':self['MAIN']['defaultWfType']}
            self.savePatientConfig()