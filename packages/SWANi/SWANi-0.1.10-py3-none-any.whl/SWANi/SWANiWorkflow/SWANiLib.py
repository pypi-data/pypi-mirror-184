...
def create_probtrackx2_pipeline(name,tract_name,resultDir):
    ...
    out_fields = ['fdt_paths_rh', 'fdt_paths_lh']
    ...
    for side in sides:
        ...
        sumTrack = Node(multiSum(),name='sumTrack_%s'%(side))
        ...
        wf.connect(track_loop[side], "fdt_paths", sumTrack, "in_files")
        wf.connect(inputnode, "ref", warp_loop[side], "ref_file")
        wf.connect(sumTrack, "out_file", warp_loop[side], "in_file")
...
class multiSumInputSpec(BaseInterfaceInputSpec):
    in_files= InputMultiPath(File(exists=True), mandatory=True, desc="list of file to sum togheter")
    out_file = File(desc='the output image')

class multiSumOutputSpec(TraitedSpec):
    out_file = File(exists=True, desc='the output image')

class multiSum(BaseInterface):
    input_spec = multiSumInputSpec
    output_spec = multiSumOutputSpec

    def _run_interface(self, runtime):
        self.inputs.out_file=self._gen_outfilename()

        steps=len(self.inputs.in_files)-1
        sum=[None] * steps
        sum_res=[None] * steps
        ...
        for x in range(steps):
            ...
            if x==0:
                sum[x].inputs.in_file=self.inputs.in_files[x]
            else:
                sum[x].inputs.in_file=sum_res[(x-1)].outputs.out_file

            sum[x].inputs.operand_file=self.inputs.in_files[(x+1)]
