...
class TimescaleDB(Service):
    ...
    def run(cls):
        ...
        if USE_DOCKER:
            ...
            inst = client.instance.instance(image=cls.singularity_container, options=[f"-B {tmpdir.name}:/var/lib/postgresql/data", f"-B {tmpdir.name}:/var/run/postgresql", f"--env POSTGRES_PASSWORD=password"])
            def _run_timescale():
