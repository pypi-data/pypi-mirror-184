...
# are self-conained
tmpdir = os.makedirs(os.getcwd() + "/tmp/")
...
class MongoDB(Service):
    ...
    def run(cls):
        ...
        if USE_DOCKER:
            ...
            inst = client.instance.instance(image=cls.singularity_container, singularity_options=["-s"], options=[f"-B {dbdir.name}:/data/db", f"-B {dbdir.name}:/tmp"])
            def _run_mongo():
...
class TimescaleDB(Service):
    ...
    def run(cls):
        ...
        if USE_DOCKER:
            ...
            inst = client.instance.instance(image=cls.singularity_container, singularity_options=["-s"], options=[f"-B {dbdir.name}:/var/lib/postgresql/data", f"-B {dbdir.name}:/var/run/postgresql", f"--env=POSTGRES_PASSWORD"])
            def _run_timescale():
