...
class Service:
    ...
    def stop(cls, instance):
        client.instance.stop(image=f"instance://{instance.name}")
...
class MongoDB(Service):
    ...
    @classmethod
    def run(cls, image_file: str = "/sw/apps/singularity-images/mines_data_engineering/mongo:latest.sif"):
        cls.pull(image_file)
        if USE_DOCKER:
...
class TimescaleDB(Service):
    ...
    @classmethod
    def run(cls, image_file: str = "/sw/apps/singularity-images/mines_data_engineering/timescaledb:latest-pg14.sif"):
        cls.pull(image_file)
        if USE_DOCKER:
