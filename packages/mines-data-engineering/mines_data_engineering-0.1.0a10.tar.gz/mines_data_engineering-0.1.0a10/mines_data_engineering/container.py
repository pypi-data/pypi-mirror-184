...
class TimescaleDB(Service):
    ...
    def run(cls):
        ...
        if USE_DOCKER:
            ...
            inst = client.instance.instance(image=self.singularity_container, options=[f"-B {tmpdir.name}:/home/postgres/pgdata/data", f"-B {tmpdir.name}:/var/run/postgresql", f"--env POSTGRES_PASSWORD=password"])
            #def _run_timescale():
            #    client.execute(image=f"instance://{inst.name}", command=["mongod"])
