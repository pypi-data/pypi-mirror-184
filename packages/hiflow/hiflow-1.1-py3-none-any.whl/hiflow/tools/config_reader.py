...
class ConfigReader:
    ...
    def register_stages(self):
        stages_dir_path = Path(utils.get_proj_dir()) / 'project' / 'stages'
    ...
    def fix_references(config_file):
        ...
        with open(config_file, 'r') as f:
            ...
            for i, line in enumerate(lines):
                lines[i] = line.replace('$HF_PROJ_PATH', utils.get_proj_dir())
            content = ''.join(lines)

        # make temp dir to keep fixed config file
        tmp_dir_path = Path(utils.get_proj_dir()) / '.tmp' / 'config.py'
