...
def create(config, plandir, cluster, overrides):
    ...
    kubeconfigfile = os.path.basename(kubeconfig) if kubeconfig is not None else 'config'
    ...
    if yaml.safe_load(os.popen('oc get crd hostedclusters.hypershift.openshift.io -o yaml 2>/dev/null').read()) is None:
        ...
        hypercmd = f"podman run -it --rm --entrypoint=/usr/bin/hypershift -e KUBECONFIG=/k/{kubeconfigfile}"
    ...
    copy2(f'{kubeconfigdir}/{kubeconfigfile}', f"{clusterdir}/kubeconfig.base")
