...
def update_envs():
    """Update Var. attributes to geezDB"""
    from .. import geezDB

    for envs in list(os.environ):
        if envs in ["LOG_CHAT", "BOT_TOKEN"] or envs in geezDB.keys():
            geezDB.set_key(envs, os.environ[envs])
...
class Helpers(object):
    ...
    def gen_chlog(self, repo, diff):
        """Generate Changelogs..."""
        from pyBetawi import ayiin_ver
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Geez Pyro {ayiin_ver} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Geez Pyro {ayiin_ver} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
