...
def main():
    ...
    net = Net.get(params['netname'], type=NetType.Analog, setup_function=net_setup, teardown_function=net_teardown)
    if params['voltdiv']:
        set_voltdiv(net, params['voltdiv'])
    if params['timediv']:
        set_timediv(net, params['timediv'])
    if params['voltoffset']:
        set_voltoffset(net, params['voltoffset']) 
    if params['timeoffset']:
        set_timeoffset(net, params['timeoffset'])                        
