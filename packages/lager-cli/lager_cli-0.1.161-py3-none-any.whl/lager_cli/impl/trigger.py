...
def trigger_uart(netname, mode, coupling, source, level, trigger_on, parity, stop_bits, baud, data_width, data):
    ...
    if trigger_on.lower() == "start":
        ...
        trig_parity = None
        trig_stop_bits = float(stop_bits)
        trig_baud = int(baud)
        trig_data = int(data)
        trig_data_width = int(data_width)
        ...
        target_net.trigger_settings.uart.set_trigger_on_data(trig_parity, trig_stop_bits, trig_baud, trig_data_width, trig_data)
        return
...
def trigger_pulse(netname, mode, coupling, source, level, trigger_on, upper, lower):
    ...
    if trigger_on.lower() == "gt":
        ...
        target_net.trigger_settings.pulse.set_trigger_on_pulse_less_than_greater_than(upper, lower)
    else:
...
def trigger_i2c(netname, mode, coupling, source_scl, level_scl, source_sda, level_sda, trigger_on, address, addr_width, data, data_width, direction):
    ...
    target_net.trigger_settings.i2c.set_source(source_scl_net, source_sda_net)
    ...
    if trigger_on.lower() == "start":
        ...
        target_net.trigger_settings.i2c.set_trigger_on_address(addr_width, direction, address) 
    elif trigger_on.lower() == "data":
        target_net.trigger_settings.i2c.set_trigger_on_data(data_width, data)
    elif trigger_on.lower() == "addr_data":
        target_net.trigger_settings.i2c.set_trigger_on_address(addr_width, direction, address, data_width, data)                                
    else:
         raise Exception(f"{trigger_on} is not a valid option")

def trigger_spi(netname, mode, coupling, source_mosi_miso, source_sck, source_cs, level_mosi_miso, level_sck, level_cs, data, data_width, clk_slope, trigger_on, cs_mode, timeout):
    target_net = Net(netname, type=None, setup_function=net_setup, teardown_function=net_teardown)
    ...
    target_net.trigger_settings.spi.set_source(source_sck_net, source_mosi_miso_net, source_cs_net)
    ...
    target_net.trigger_settings.spi.set_trigger_data(data_width, data)
