...
def get_git_commit_date(path: str) -> Optional[str]:
    """
    Obtains the date of the latest commit on the current branch of the git repository associated
    with the specified path, returning ``None`` if the path does not correspond to a git
    repository.
    """
    try:
        ...
        _logger.warning(
            "Failed to import Git (the Git executable is probably not on your PATH),"
            " so Git Commit Date is not available. Error: %s",
            e,
        )
        return None
    try:
