    ...
    def __init__(self, args):
        for key, value in args.items():
            if isinstance(value, dict):
                SubModel = ModelAbstract()
                SubModel.__dict__.update(value)
                value = SubModel
...
class Wizard(object):
    ...
    def run(self, method, view, ctx={}):
        ...
        with Transaction().start(self.db, user=user, context=ctx):
            ...
            prueba = getattr(_self_wiz, start_state)
            print(prueba, dir(prueba))
            setattr(_self_wiz, start_state, StartState(args))
