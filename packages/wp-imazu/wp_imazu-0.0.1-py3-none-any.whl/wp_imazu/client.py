...
class ImazuClient(WpSocketClient):
    ...
    def __init__(self, host: str, port: int, async_device_state_handler):
        super().__init__(host, port, self._async_packets_handler)
        self.async_device_state_handler = async_device_state_handler
    ...
    async def async_on_connected(self):
        ...
        # t = "f70b011f0140110000b2ee"
        # tt = bytes.fromhex(t)
        # await self.async_send_packet(tt)
    ...
    def _parse_packets(self, packets):
        ...
        while packets:
            start_idx = packets.find(PACKET_HEADER)
            if start_idx == -1:
            ...
            end_idx = packets.find(PACKET_TAIL, start_idx + 8)
            if end_idx == -1:
