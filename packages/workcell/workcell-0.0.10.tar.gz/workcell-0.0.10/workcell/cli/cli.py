...
from workcell.core.constants import SCAFFOLD_FOLDER, RUNTIME_FOLDER
    ...
    try:    
        ...
        if (os.getenv("WORKCELL_USERNAME") == username) and ('WORKCELL_TOKEN' in os.environ) and (os.getenv("WORKCELL_TOKEN") != ""):
            ...
            if (username == "") and (access_token == ""):
                ...
                # login url
                url = os.getenv("WORKCELL_GATEWAY") + "/auth/token"
                # login request   
...
def deploy(
    ...
    try:    
        if ('WORKCELL_TOKEN' in os.environ) and (os.getenv("WORKCELL_TOKEN") != ""):
            ...
            # auth url 
            url = os.getenv("WORKCELL_GATEWAY") + "/workcell/deploy"
...
def teardown(
    ...
    try:    
        if ('WORKCELL_TOKEN' in os.environ) and (os.getenv("WORKCELL_TOKEN") != ""):
            ...
            # auth url 
            url = os.getenv("WORKCELL_GATEWAY") + "/workcell/teardown"
            # post to teardown
            response = requests.post(
                url=url, 
                headers=headers, 
            ...
            if response.status_code == 200:
                typer.echo("Workcell teardown success, response text: {}".format(response.text))    
            else:
