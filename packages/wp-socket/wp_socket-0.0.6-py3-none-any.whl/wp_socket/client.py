...
class WpSocketClient:
    ...
    async def async_on_connected(self):
        ...
        _LOGGER.debug('connected {}', self._host)

    async def async_connect(self) -> bool:
        """Socket connect"""
        await self.async_wait_for_disconnect()

        _LOGGER.debug('connecting to server {}', {self._host})
        try:
            ...
            _LOGGER.error('connection error, {}, {}', self._host, ex)
            self.disconnect()
        return False
    ...
    async def _async_reconnect(self):
        ...
        _LOGGER.debug('reconnect connect, wait {}s', wait_time)
    ...
    async def _async_reader_handler(self):
        ...
        while self._connected:
            try:
                ...
                _LOGGER.error('message handler error, {}', ex)
    ...
    async def _async_reader(self):
        ...
        while self._connected:
            try:
                data = await asyncio.wait_for(self._reader.read(_DEFAULT_READ_BUFFER),
                                              timeout=_READ_TIMEOUT)
                if not data:
                    self._loop.create_task(self._async_reconnect())
                    break

                _LOGGER.debug('Received [{}]: {}', len(data), data.hex())
                await self._receive_packet_queue.put(data)
            except Exception as ex:
                _LOGGER.error('reader error, {}', ex)
                self._loop.create_task(self._async_reconnect())
                break
    ...
    async def _async_writer(self):
        ...
        while self._connected:
            try:
                ...
                _LOGGER.debug('Send [{}]: {}', len(packet), packet.hex())
                self._send_packet_queue.task_done()
            except asyncio.TimeoutError:
                continue
            except Exception as ex:
                _LOGGER.error('writer error, {}', ex)
                self._loop.create_task(self._async_reconnect())
                break
