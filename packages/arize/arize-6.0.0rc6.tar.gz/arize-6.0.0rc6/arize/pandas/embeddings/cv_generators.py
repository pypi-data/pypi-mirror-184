...
class EmbeddingGeneratorForCVImageClassification(CVEmbeddingGenerator):
    ...
    def generate_embeddings(self, image_path_col: pd.Series):
        ...
        if image_path_col.isnull().any():
            raise ValueError(f"There can't be any null values in the column: {image_path_col}")

        ds = Dataset.from_pandas(pd.DataFrame(image_path_col))
        ds.set_transform(
            partial(
                self.__extract_image_features, image_path_col_name=image_path_col.name
            )
        )
    ...
    def __extract_image_features(self, batch, image_path_col_name):
        return self.feature_extractor(
            [
                self.__process_image(image_path)
