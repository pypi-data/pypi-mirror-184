...
def main(target, ground, output, mappings=None, variables=None, authtype="pba"):
    """A tool to update scribe competency configurations."""
    try:    
        target = make_lower(load_json(target))
        ground = make_lower(load_json(ground))

        if os.path.isfile(f"version_mapping/{mappings}"):
            version_map = load_json(f"version_mapping/{mappings}")
