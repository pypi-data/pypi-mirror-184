...
class ChangeLog:
    ...
    def check_for_removed_competencies_scenarios(self):
        for competency in self.target_scenarios:
            ...
            if competency not in self.ground_scenarios and not base_mapped and competency not in ["disabled", "out_of_scope"]:
        ...
        for competency in self.ground_scenarios:
            if competency not in self.target_scenarios:
                ...
                for scenario in self.ground_scenarios[competency]:
                    ...
                    if scenario not in self.target_scenarios[competency] and output_key not in self.ground_to_target_map.keys():
    ...
    def check_competency(self, competency, target_competency):
        # Changing below to show customer relevant changes only
        field_names = ["disabled"]
        # field_names = ["type", "disabled", "templates"]
    ...
    def check_scenario(self, competency_name, output_scenario, target_scenario):
        ...
        # field_names = ["description", "example_queries", "disabled", "tags", "cases", "response_elements"]
        for i in field_names:
    ...
    def write_change_log_to_file(self):
        with open("diff_log.json", "w") as f:
