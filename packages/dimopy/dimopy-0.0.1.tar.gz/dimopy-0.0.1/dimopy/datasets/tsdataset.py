...
A TSDataset object comprises two kinds of time series data:
...
class TimeSeries(object):
    ...
    @classmethod
    def concat(cls, tss: List["TimeSeries"], axis: int = 0, drop_duplicates: bool = True,
        ...
        if axis == 0:
            ...
            if drop_duplicates:
                ...
                if isinstance(tss[0].data.index, pd.RangeIndex):
                    ...
                    data = data.set_index(
                        pd.RangeIndex(data.index.values[0], data.index.values[-1] + tss[0].freq, tss[0].freq))
            else:
                raise_if(
...
class TSDataset(object):
    ...
    def plot(self,
        ...
        if add_data:
            ...
            if labels:
                ...
                raise_if(len(custome_labels) != len(add_data),
    ...
    def _fill_between_quantiles(self,
        ...
        plt.fill_between(self.target.data.index,
                         ...
                         label=str(int(low_quantile * 100)) + "%-" + str(
                             int(high_quantile * 100)) + "% probability interval"
                         )
    ...
    def concat(cls, tss: List["TSDataset"], axis: int = 0, drop_duplicates=True, keep='first') -> "TSDataset":
        ...
        target = TimeSeries.concat(targets, axis, drop_duplicates=drop_duplicates, keep=keep) if len(
            targets) != 0 else None
        known_covs = [ts.get_known_cov() for ts in tss if ts.get_known_cov() is not None]
        known_cov = TimeSeries.concat(known_covs, axis, drop_duplicates=drop_duplicates, keep=keep) if len(
            known_covs) != 0 else None
        observed_covs = [ts.get_observed_cov() for ts in tss if ts.get_observed_cov() is not None]
        observed_cov = TimeSeries.concat(observed_covs, axis, drop_duplicates=drop_duplicates, keep=keep) if len(
            observed_covs) != 0 else None
        static_cov = {}
        for ts in tss:
