...
from dimopy.datasets import TSDataset
from dimopy.logger import Logger, raise_if, raise_if_not
    ...
    def split(self, dataset: TSDataset,
        ...
        for train, test in splits:
            if isinstance(indices, pd.DatetimeIndex):
                ...
                train_index = pd.RangeIndex(indices[0] + train[0] * indices.step,
                                            indices[0] + train[-1] * indices.step + indices.step, indices.step)
                test_index = pd.RangeIndex(indices[0] + test[0] * indices.step,
                                           indices[0] + test[-1] * indices.step + indices.step, indices.step)
            if return_index:
