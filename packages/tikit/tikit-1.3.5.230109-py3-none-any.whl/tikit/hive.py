...
from tikit.config import EMR_SITE_TEMPLATE
    ...
    def _get_emr_security_auth_info(self, emr_id, username, keytab=None):
        ...
        for v in conf_rsp.ServiceConfList:
            ...
            if v.FileName == "core-site.xml":
                core_site = self._get_site_xml(tmp_param, EMR_SITE_TEMPLATE["core-site.xml"])
                if core_site:
                    self._save_xml(core_site, self._core_site)
                continue

            if v.FileName == "yarn-site.xml":
                yarn_site = self._get_site_xml(tmp_param, EMR_SITE_TEMPLATE["yarn-site.xml"])
                if yarn_site:
            ...
            if v.FileName == "hive-site.xml":
                ...
                hive_site = self._get_site_xml(tmp_param, EMR_SITE_TEMPLATE["hive-site.xml"])
                if hive_site:
                    if "hive.metastore.uris" not in hive_site:
                        hive_site["hive.metastore.uris"] = "thrift://{}:7004".format(master_host)
...
[domain_realm]""" % (realm.upper(), kdc + ":" + port, admin_server)
    ...
    def _get_site_xml(self, params, config):
        ret = {}
        for v in params:
            if v["Name"] in config:
                if config[v["Name"]] is None:
                    ret[v["Name"]] = v["Value"]
                else:
                    ret[v["Name"]] = config[v["Name"]]
        return ret
