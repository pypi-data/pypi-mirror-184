...
def set_build_time_stamp():
    with open('asd/__init__.py','r') as fw: lines = fw.readlines()
    __doc__ = '__built_time__="built at {} {}"\n'.format(time.ctime(),time.tzname[1])
    lines = [__doc__] + [line for line in lines if '__built_time__' not in line]
    with open('asd/__init__.py','w') as fw: fw.write(''.join(lines))

 
 
if __name__=='__main__':
    set_build_time_stamp()
    print('\n{0}\nINSTALL\n{0}'.format('='*50))
