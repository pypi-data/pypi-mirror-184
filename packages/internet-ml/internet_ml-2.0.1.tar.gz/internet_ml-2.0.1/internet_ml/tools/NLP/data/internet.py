...
class Google:
    ...
    def __get_urls_contents(self: "Google") -> None:
        ...
        tmp_content = []
        for content in self.__content:
            tmp_content.append(filter_irrelevant(content))
        self.__content = tmp_content
    ...
    def google(self: "Google") -> tuple[list[str], list[str]]:
        ...
        try:
            ...
            cache = {}
        # Check if any of the keywords are in the cache
        keywords = get_keywords(self.__query)
        for keyword in keywords:
            if keyword in cache:
                results_cache: Tuple[List[str], List[str]] = cache[keyword]
