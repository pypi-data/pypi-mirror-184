...
class Model(object):
    ...
    def fetchTopology(
        ...
        ## 电磁暂态运行内核入口
    def emtpRun(self,job=None,config=None)->Runner[EMTResult]:
        '''
            运行 emtp 内核，如果当前 model 没有创建  Job 时报错，
            默认使用第一个计算方案，进行仿真。
    ...
    ## 移频电磁暂态内核运行入口
    def sfemtRun(self,job=None,config=None)->Runner[EMTResult]:
        '''
            运行 移频电磁暂态 内核，如果当前 model 没有创建  Job 时报错，
            默认使用第一个计算方案，进行仿真。
            :param: job 计算方案名称，可选，字符串类型或者字典类型,默认使用第一个计算方案，如果同名使用最靠前一个
            :param: config 参数方案，可选，字符串类型或者字典类型,默认使用保存时选中的参数方案
    ...
    ## 潮流计算内核运行入口
    def powerFlowRun(self,job=None,config=None)->Runner[PowerFlowResult]:
        '''
            运行 潮流 内核，如果当前 model 没有创建  Job 时报错，
            默认使用第一个计算方案，进行仿真。
            :param: job 计算方案名称，可选，字符串类型或者字典类型,默认使用第一个计算方案，如果同名使用最靠前一个
            :param: config 参数方案，可选，字符串类型或者字典类型,默认使用保存时选中的参数方案
    ...
    ## 配电网三相不平衡潮流内核运行入口
    def threePhasePowerFlowRun(self,job=None,config=None)->Runner[PowerFlowResult]:
        '''
            运行 批量潮流 内核，如果当前 model 没有创建  Job 时报错，
            默认使用第一个计算方案，进行仿真。
            :param: job 计算方案名称，可选，字符串类型或者字典类型,默认使用第一个计算方案，如果同名使用最靠前一个
            :param: config 参数方案，可选，字符串类型或者字典类型,默认使用保存时选中的参数方案
            ...
            if job['rid'] != 'job-definition/cloudpss/three-phase-powerFlow':
                for j in self.jobs:
                    if j['rid'] == 'job-definition/cloudpss/three-phase-powerFlow':
                        job = j
        if job is None:
            raise Exception("找不到批量潮流内核运行的计算方案")
        if job['rid'] != 'job-definition/cloudpss/three-phase-powerFlow':
            raise Exception("不是批量潮流内核运行生成算法的计算方案")
        if config is None:
            currentConfig = self.context['currentConfig']
            config = self.configs[currentConfig]
        return self.run(job=job, config=config)
