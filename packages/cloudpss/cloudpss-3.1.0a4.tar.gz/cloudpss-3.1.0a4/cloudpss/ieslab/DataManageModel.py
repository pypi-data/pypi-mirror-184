...
class DataManageModel(object):
    ...
    def AddDataItem(self, dataType, data):
        '''
            ...
            :return: string类型，返回新添加数据项的timeID，如果数据结构不满足要求，应当抛出异常
        '''
        assert (dataType in self._kindNameMap
                or dataType in self._kindUrlMap), "数据类型不存在"
        kind = self._kindNameMap.get(dataType, dataType)
        if data['id'] is not None:
            del data['id']
        if data['timeid'] is not None:
            del data['timeid']
        try:
            ret = self._saveItemData(self._kindUrlMap[kind], data)
            if kind in ['thermalLoads', 'electricLoads', 'fuels']:
                dataList = self._fetchItemData(self._kindUrlMap[kind], None)
                return dataList[len(dataList) - 1]['timeid']
            else:
                dataList = self._fetchItemData(self._kindUrlMap[kind], kind)
                return dataList[len(dataList) - 1]['timeid']
    ...
    def UpdateDataItem(self, dataType, data):
        '''
            修改dataType类型的数据库中添加内容为data的数据项
            
            :params: dataType enum类型，数据的种类标识，比如Transformer, Load, HeatPrice等等
            :params: data dict类型，表示添加的数据内容，其数据结构应满足对应数据项的结构要求
            
            :return: bool 类型，修改是否成功
        '''
        assert (dataType in self._kindNameMap
                or dataType in self._kindUrlMap), "数据类型不存在"
        try:
            kind = self._kindNameMap.get(dataType, dataType)
            url=self._kindUrlMap[kind] + '/' + str(data['id']) + '/'
