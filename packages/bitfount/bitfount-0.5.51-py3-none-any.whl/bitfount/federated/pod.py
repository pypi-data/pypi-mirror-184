...
class Pod:
    ...
    def _update_pod_db(self) -> None:
        ...
        :::note
        Does not work for DatabaseSource.
        :::
        """
        if self.datasource.multi_table:
            ...
            # get data for single table datasources
            # always returns a df, so we cna cast
            new_data = cast(pd.DataFrame, self.datasource.get_data()).copy()
            _add_data_to_pod_db(pod_name=self.name, data=new_data, table_name=self.name)
