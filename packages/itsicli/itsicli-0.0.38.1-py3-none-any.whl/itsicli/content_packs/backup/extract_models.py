...
class Extractor(object):
    ...
    def remove_cp_id_prefix(self, to_be_cleaned):
        ...
        if to_be_cleaned.startswith('da-itsi-cp-'):
            updated_string = re.sub(r'da-itsi-cp-([\w]+?)-', '', to_be_cleaned)
        elif to_be_cleaned.startswith('da-itsi-cp-cust-'):
            updated_string = re.sub(r'da-itsi-cp-cust-([\w]+?)-', '', to_be_cleaned)
...
class GlassTableIconExtractor(GlassTableImageExtractor):
    ...
    def apply_prefix(self, raw_data):
        ...
        # IconService. It is used during creation of
        # icons.(i.e. install of CP))
        # this means if two icons have different '_key' but
        # the same title and category, only one will be
        # saved into the kv collection.
        #
        # another caveat with icons is, backup on ITSI sie
        # always include all the icons as long as glass
        # table is included. (regardless of full/partial backup)
        # it is best if we don't install entire icon collection
        # on a system if multiple CPs are installed
        # with glasstables.
        # 1. immutable icons: because the 'same' icon will be on destination
        # system. we need to have prefixed-title and _key
        # so they don't conflict on destination system.  it will be
        # a common prefix to prevent repeat installation and also
        # this allows the references to work from CP and all CP
        # during install, we will skip over if same icon is found
        #
        # 2. regular icons: usually shouldn't be on the destination
        # but because all icons are always packaged and is not CP specific
        # We will hit 'repeats' as soon as we install another CP with glass table.
        # in order to prevent multiple copies of 'same' icon installed on destination
        # we will also prefix their title/id with common prefix
        #
        # in conclusion: we will prefix all icons (immutable or not) with
        # common prefix for title and key. we will skip them if
