...
# Will return the object carrying the biggest attribute name from a list.
def get_object_with_max_value(obj_list, attribute_name, def_return=None):
    if not obj_list:
        return def_return
    max_item = max(obj_list, key=lambda item: getattr(item, 'padding'))
    return max_item


# Will return the maximum value from a list of objects by attribute name
def get_max_value_from_obj_list(obj_list, attribute_name, def_return=None):
    biggest_obj = get_object_with_max_value(obj_list, attribute_name, def_return)
    if biggest_obj == def_return:
        return def_return
    ans2 = getattr(biggest_obj, 'padding')
    return ans2
