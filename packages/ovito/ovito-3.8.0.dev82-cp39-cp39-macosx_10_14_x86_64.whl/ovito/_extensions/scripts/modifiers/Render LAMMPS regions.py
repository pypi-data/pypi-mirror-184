...
class Cone(RegionStyle):
    ...
    def parse_style_parameters(self, tokens: List[str]):
        ...
        try:
            self.c1, self.c2, self.radius1, self.radius2, self.lo, self.hi = list(map(lambda v: float(v) if v not in ('INF', 'EDGE') else v, tokens[1:7]))
        except Exception as exc:
    ...
    def generate_verts_and_faces(self, domain: SimulationCell):
        ...
        bottom_plane = Cylinder.points_on_circle(self.radius1, self.resolution, self.dim)
        top_plane = Cylinder.points_on_circle(self.radius2, self.resolution, self.dim)
...
class RenderLAMMPSRegionsModifier(ModifierInterface):
    ...
    def modify(self, data: DataCollection, **kwargs):
        ...
        # Create particle selection of particles inside mesh region
        if self.select_atoms:
            selection = data.particles_.create_property('Selection')
            try:
                for index, pos in enumerate(data.particles.positions):
                    yield (index/data.particles.count)
                    region = surface_mesh.locate_point(pos)
                    if region != -1 and region is not None:
                        selection[index] = 1
            except:
                print('Warning: Particle selection not available for surface meshes that are not closed.')
