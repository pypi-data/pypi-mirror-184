...
def _hiscores(player: str, path: str):
    try:
        with util.request(path, player=player) as resp:
            return util.parse_scores(resp.text, SKILLS)
...
def news() -> dict:
    with util.request(NEWS_PATH, oldschool=True) as resp:
        return util.parse_news(resp.text)


def _items_iter(query: typing.Union[int, str]) -> typing.Iterable[wiki.Item]:
    if query is None:
...
def _ge_get(id_):
    with util.request(GE_PATH, item=id_) as resp:
