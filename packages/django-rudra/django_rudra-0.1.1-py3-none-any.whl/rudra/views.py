...
from rudra.service import get_model, get_models, get_serializer, get_success_response, get_error_response, check_method_allowed, get_serializer_context
...
class QueryModel(APIView):
    ...
    def get(self, request: HttpRequest, model_name: str):
        ...
        if all:
            ...
            paginator = PaginatorService(instance, self.serializer, serializer_context=self.serializer_context, page=page or 1, page_size=page_size or 10)
            return get_success_response(paginator.get_serialized_data(), paginator.has_next())
        else:
