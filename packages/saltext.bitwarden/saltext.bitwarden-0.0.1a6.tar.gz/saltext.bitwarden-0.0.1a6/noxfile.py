...
PYTHON_VERSIONS = ("3", "3.6", "3.7", "3.8", "3.9", "3.10")
...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/bitwarden/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/bitwarden/*")
            ...
            #    "--include=src/saltext/bitwarden/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/bitwarden/config/schemas",
    )
