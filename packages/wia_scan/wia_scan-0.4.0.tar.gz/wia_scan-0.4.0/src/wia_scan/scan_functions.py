...
def scan_side(device, settings, print_function):
    """
...
def scan_single_side_main(print_function):
...
def scan_many_documents_flatbed(print_function):
    device_manager = get_device_manager()
    device_identifier = prompt_choose_device(
        device_manager=device_manager, print_function=print_function)
    device = connect_device(device_manager=device_manager,
    ...
    while True:
        ...
        for side in range(number_of_sides):
            if side > 0:
                ...
                press_any_key_to_continue(print_function=print_function)
