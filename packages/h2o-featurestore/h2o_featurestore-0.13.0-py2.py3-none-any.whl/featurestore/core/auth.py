...
class AuthWrapper:
    ...
    def set_obtain_access_token_method(self, method):
        """Sets a valid user access token.

        Obtains an access token from the external environment through the method and
        uses it for authentication.

        Args:
            method: (Callable) A method that returns token from the external environment.
    ...
    def login(self, open_browser=True):
        ...
        Opens the returned URL in the browser (if it's not possible to open the browser, the user
        has to do this manually) and waits for the refresh token. Returned refresh tokens will be
        saved into the client’s configuration file.

        Args:
            open_browser: (bool) If True, opens the URL in the browser. Default is True.
    ...
    def set_auth_token(self, token):
        """Sets the user token for authentication.

        Args:
            token: (str) A user's personal access token.
