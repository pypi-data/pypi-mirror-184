...
class Schema:
    ...
    @staticmethod
    def create_from(obj):
        """Creates a schema from String and FeatureSet.
    ...
    def create_derived_from(schema_string: str, derive_from, transformation: Transformation):
        """Creates a schema from a string formatted schema and parent feature set along with transformation.
    ...
    def select(self, feature_names):
        """Creates a new schema by column selection.
    ...
    def to_string(self):
        """Serializes the schema to string format."""
        return ", ".join(
            [
