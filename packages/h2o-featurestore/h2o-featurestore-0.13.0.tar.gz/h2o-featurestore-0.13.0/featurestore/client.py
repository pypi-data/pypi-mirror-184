...
class Client:
    ...
    def close(self):
        """Closes the current working channel.

        Good practice to close the connection after all action has proceeded.
        """
        self._channel.close()

    def get_version(self) -> str:
        """Returns Feature Store client version as string."""
        request = Empty()
    ...
    def extract_schema_from_source_async(self, raw_data_location, credentials=None) -> ExtractSchemaJob:
        """Creates the schema extract job.

        Generates the new job for schema extraction from provided data source.
    ...
    def extract_schema_from_source(self, raw_data_location, credentials=None) -> Schema:
        """Extracts schema from the provided data source.
    ...
    def extract_derived_schema(self, feature_sets, transformation) -> Schema:
        """Creates schema from an existing feature set using selected transformation.
    ...
    def extract_derived_schema_async(self, feature_sets, transformation) -> Schema:
        ...
        Generates the new job for schema extraction from an existing feature set using
    ...
    def show_progress(self, interactive):
        """Enables or disables interactive logging. Logging is enabled by default.
