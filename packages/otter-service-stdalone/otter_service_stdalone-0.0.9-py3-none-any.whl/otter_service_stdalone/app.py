...
class Userform(tornado.web.RequestHandler):
    async def get(self):
        if self.get_argument('code', False):
            log.write_logs("6.", str(self.get_current_user()), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
            user = await self.get_authenticated_user(
                            redirect_uri='http://grader-dev.data8x.berkeley.edu',
                            code=self.get_argument('code'))
            # Save the user with e.g. set_secure_cookie
            log.write_logs("7.", str(user), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
            self.set_secure_cookie("user", user)
            self.redirect("/")
        else:
            log.write_logs("1.", str(self.get_current_user()), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
