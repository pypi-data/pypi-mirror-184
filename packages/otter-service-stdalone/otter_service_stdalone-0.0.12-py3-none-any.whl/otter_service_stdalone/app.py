...
class GoogleOAuth2LoginHandler(tornado.web.RequestHandler,
    ...
    async def get(self):
        log.write_logs("3. and 6.", str(self.get_current_user()), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
        log.write_logs("3. and 6.", str(self.get_argument('code', False)), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')    
        if not self.get_argument('code', False):
            log.write_logs("4.", str(self.get_current_user()), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
            self.authorize_redirect(
                redirect_uri='http://grader-dev.data8x.berkeley.edu',
...
class Userform(tornado.web.RequestHandler,
    ...
    async def get(self):
        ...
        log.write_logs("1a.", str(self.get_current_user()), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
        log.write_logs("1b.", str(code), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
        if not self.get_current_user():
            log.write_logs("2, 5.", str(code), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
            red = "/login"
            if code:
                log.write_logs("5.", str(code), "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
                red = f"{red}?code={code}"
            self.redirect(red)
        
        if self.get_current_user():
            log.write_logs("7.", "Authenticated!", "auth", f'{os.environ.get("ENVIRONMENT")}-logs')
...
settings = {
    ...
    "google_oauth" : {"key": "263483406093-5hfi9fqqp3a66hdpur9uu4fov701rjcq.apps.googleusercontent.com"}
}
