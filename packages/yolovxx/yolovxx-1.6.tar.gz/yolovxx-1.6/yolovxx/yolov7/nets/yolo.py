    ...
    def __init__(self, c1, c2, k=3, s=1, p=None, g=1, act=SiLU(), deploy=False):
        ...
        if deploy:
            ...
            self.rbr_identity = (nn.BatchNorm2d(
                num_features=c1, eps=0.001, momentum=0.03) if c2 == c1 and s == 1 else None)
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False):
        ...
        #   定义了不同yolov7版本的参数
        #-----------------------------------------------#
        transition_channels = {'tiny': 16, 'l': 32, 'x': 40}[phi]
        block_channels = {'tiny': 16, 'l': 32, 'x': 32}[phi]
        panet_channels = {'tiny': 16, 'l': 32, 'x': 64}[phi]
        e = {'tiny': 1, 'l': 2, 'x': 1}[phi]
        n = {'tiny': 2, 'l': 4, 'x': 6}[phi]
        ids = {'tiny': [-1, -2, -3, -4],
               'l': [-1, -2, -3, -4, -5, -6],
               'x': [-1, -3, -5, -7, -8]}[phi]
        conv = {'tiny': Conv, 'l': RepConv, 'x': Conv}[phi]
        #-----------------------------------------------#
        #   输入图片是640, 640, 3
        #-----------------------------------------------#
        ...
        self.sppcspc = SPPCSPC(transition_channels * 32,
                               transition_channels * 16)
        if phi == "t":
            self.sppcspc = SPPCSPC_tiny(transition_channels * 32,
                               transition_channels * 16)

        self.conv_for_P5 = Conv(transition_channels *
                                16, transition_channels * 8)
        
        
        self.conv_for_feat2 = Conv(
            transition_channels * 32, transition_channels * 8)
        
        if phi == "t":
            self.conv_for_feat2 = Conv(
                transition_channels * 16, transition_channels * 8)

        self.conv3_for_upsample1 = Multi_Concat_Block(
            transition_channels * 16, panet_channels * 4, transition_channels * 8, e=e, n=n, ids=ids)

        self.conv_for_P4 = Conv(transition_channels * 8,
                                transition_channels * 4)
        
        
        self.conv_for_feat1 = Conv(
            transition_channels * 16, transition_channels * 4)
        
        if phi == "t":
            self.conv_for_feat1 = Conv(
                transition_channels * 8, transition_channels * 4)
        ...
        if phi == "t":
            self.down_sample1 = Conv(
                transition_channels * 4, transition_channels * 8, k=3, s=2)
        ...
        if phi == "t":
            self.down_sample2 = Conv(
                transition_channels * 8, transition_channels * 16, k=3, s=2)
        ...
        self.rep_conv_1 = conv(transition_channels * 4,
                               transition_channels * 8, 3, 1)
        self.rep_conv_2 = conv(transition_channels * 8,
                               transition_channels * 16, 3, 1)
        self.rep_conv_3 = conv(transition_channels * 16,
                               transition_channels * 32, 3, 1)
