    ...
    def createProjectFromConfig(self, config):
        ...
        if "plugins" in config:
            for plugin in config["plugins"]:
                ...
                pluginClass = importlib.import_module(".Plugin", package=plugin["name"]).__file__
                defaultConfig = Path(pluginClass).parent.joinpath("config.yaml")
                if defaultConfig.exists():
                    pluginConfig = self.loadConfig(defaultConfig)
                    project.config.setdefaults(pluginConfig["config"])
