  ...
  <thead>
    <tr>
      ...
      % if entity.rankedturn:
        ...
        <th class="center aligned event-header">${_('Tot')}</th>
      % endif
...
<%def name="boards_body()">
  <tbody>
    ...
    <tr>
      <td class="right aligned${match.score1>match.score2 and ' winner' or ''}">
        ${match.score1}
      </td>
      <td colspan="6"></td>
      <td class="right aligned${match.score1<match.score2 and ' winner' or ''}">
...
<table class="ui striped compact unstackable table boards">
  <caption>
    ...
    </a>, ${_('table $num', mapping=dict(num=match.board))}
    <%
...
<%def name="matches_body(matches)">
  <tbody>
    % for i, row in enumerate(matches, 1):
    ${matches_row(i, row, any(m.boards for m in matches))}
    % endfor
  </tbody>
</%def>

<%def name="matches_row(rank, row, show_details)">
  <%
