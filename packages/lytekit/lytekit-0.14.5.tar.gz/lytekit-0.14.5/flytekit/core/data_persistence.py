...
class DataPersistence(object):
    ...
    @abstractmethod
    def put(self, from_path: str, to_path: str, recursive: bool = False) -> Optional[Dict[str, Any]]:
        """
        ...
        if protocol in cls._PLUGINS:
            ...
            if not force:
                raise TypeError(
                    ...
                    " registered for the same protocol. You can force register the new plugin by passing force=True"
                )
    ...
    def put_data(self, local_path: Union[str, os.PathLike], remote_path: str, is_multipart=False):
        ...
        try:
            with PerformanceTimer(f"Writing ({local_path} -> {remote_path})"):
                return DataPersistencePlugins.find_plugin(remote_path)().put(
