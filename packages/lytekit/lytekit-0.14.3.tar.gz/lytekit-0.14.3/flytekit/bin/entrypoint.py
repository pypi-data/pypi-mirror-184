...
def _dispatch_execute(
    ctx: FlyteContext, task_def: PythonTask, inputs_path: str, output_prefix: str,
):
    """
...
def setup_execution(
    ...
    execution_parameters = ExecutionParameters(
        execution_id=_identifier.WorkflowExecutionIdentifier(project=exe_project, domain=exe_domain, name=exe_name,),
    ...
    try:
        file_access = FileAccessProvider(
            local_sandbox_dir=tempfile.mkdtemp(prefix="flyte"), raw_output_prefix=raw_output_data_prefix,
        )
    ...
    es = ctx.new_execution_state().with_params(
        mode=ExecutionState.Mode.TASK_EXECUTION, user_space_params=execution_parameters,
    )
    ...
    if compressed_serialization_settings:
        ...
        if dynamic_addl_distro:
            ssb.fast_serialization_settings = FastSerializationSettings(
                enabled=True, destination_dir=dynamic_dest_dir, distribution_location=dynamic_addl_distro,
            )
...
def _handle_annotated_task(
    ctx: FlyteContext, task_def: PythonTask, inputs: str, output_prefix: str,
):
    """
