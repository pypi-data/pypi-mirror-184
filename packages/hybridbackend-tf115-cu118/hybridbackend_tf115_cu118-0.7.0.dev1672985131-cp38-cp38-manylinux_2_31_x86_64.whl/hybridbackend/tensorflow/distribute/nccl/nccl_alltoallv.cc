...
struct NcclAlltoallvCall<float, Eigen::half> {
  Status operator()(const Tensor& input, const int32* send_counts,
    ...
    VLOG(1) << comm->DebugString() << " [" << comm_op->name()
            << "] [Alltoallv]";
    TF_RETURN_IF_ERROR(comm->Alltoallv(*comm_input, send_counts, recv_counts,
                                       common_size, comm_output));
...
struct NcclAlltoallvNCall {
  Status operator()(const std::vector<Tensor>& n_input,
    ...
    VLOG(1) << comm->DebugString() << " [" << comm_op->name()
            << "] [AlltoallvN]";
    TF_RETURN_IF_ERROR(comm->AlltoallvN(
        n_input, n_send_sizes_vec, n_recv_sizes_vec, n_common_size, n_output));
    return Status::OK();
  }
};
...
struct NcclAlltoallvNCall<float, Eigen::half> {
  Status operator()(const std::vector<Tensor>& n_input,
    ...
    VLOG(1) << comm->DebugString() << " [" << comm_op->name()
            << "] [AlltoallvN]";
    TF_RETURN_IF_ERROR(comm->AlltoallvN(*n_comm_input, n_send_sizes_vec,
...
class NcclAlltoallvOp : public NcclCommAsyncOp {
 public:
  ...
  virtual void ComputeAsyncWithComm(NcclComm* comm, OpKernelContext* ctx,
    ...
    TensorShape output_sizes_shape({comm->world_size()});
    Tensor* output_sizes;
    ...
    OP_REQUIRES_OK_ASYNC(
        ctx,
        ctx->allocate_temp(DT_INT32, TensorShape({comm->world_size()}),
    ...
    comm->stream()->LaunchUntilComputeDone(
        ctx, [input_sizes, output_sizes, host_input_sizes, host_output_sizes,
              input, comm_input, comm_output, ctx, comm, this, done_]() {
          ...
          OP_REQUIRES_OK_ASYNC(ctx, comm->Alltoall(*input_sizes, output_sizes),
                               done_);
          comm->stream()->BlockComputeUntilDone(ctx);
          ctx->op_device_context()->stream()->ThenMemcpy(
          ...
          for (int32 rank = 0; rank < comm->world_size(); ++rank) {
          ...
          OP_REQUIRES_OK_ASYNC(
              ...
              call(*input, host_input_sizes->flat<int32>().data(),
                   ...
                   output, comm_input, comm_output, ctx, comm, this),
              done_);
 ...
 public:
  ...
  virtual void ComputeAsyncWithComm(NcclComm* comm, OpKernelContext* ctx,
    ...
    for (int idx = 0; idx < N_; ++idx) {
      ...
      OP_REQUIRES_ASYNC(
          ctx, input_sizes.NumElements() == comm->world_size(),
          errors::InvalidArgument(
              "Sizes of input ", idx, " has ", input_sizes.NumElements(),
              " elements, which is not equal to communicator world size: ",
              comm->world_size()),
          done_);
      ...
      OP_REQUIRES_OK_ASYNC(
          ctx,
          ctx->allocate_output(N_ + idx, {comm->world_size()}, &output_sizes),
          done_);
      n_output_sizes->push_back(output_sizes);
    }

    comm->stream()->LaunchUntilComputeDone(
        ctx, [this, comm, ctx, done_, n_input, n_input_sizes, n_output_sizes,
              n_host_input_sizes, n_host_output_sizes, n_comm_input,
              n_comm_output]() {
          ...
          VLOG(1) << comm->DebugString() << " [" << name() << "] [AlltoallN]";
          OP_REQUIRES_OK_ASYNC(
              ctx, comm->AlltoallN(*n_input_sizes, n_output_sizes), done_);
          comm->stream()->BlockComputeUntilDone(ctx);
          for (size_t idx = 0; idx < N_; ++idx) {
            ctx->op_device_context()->stream()->ThenMemcpy(
                const_cast<char*>(
          ...
          OP_REQUIRES_OK_ASYNC(
              ...
              call(*n_input, *n_host_input_sizes, *n_host_output_sizes,
                   common_sizes_, &n_output, n_comm_input, n_comm_output, ctx,
                   comm, this),
              done_);
          comm->stream()->BlockComputeUntilDone(ctx, done_);
        });
...
#if GOOGLE_CUDA
template <typename DTYPE, typename WIRE_DTYPE>
class NcclAlltoallvMergedNOp : public NcclCommAsyncOp {
 public:
  ...
  virtual void ComputeAsyncWithComm(NcclComm* comm, OpKernelContext* ctx,
    ...
    for (int idx = 0; idx < N_; ++idx) {
      ...
      OP_REQUIRES_OK_ASYNC(
          ...
          done_);
      n_output_sizes->push_back(output_sizes);
      const auto input_bytes = DataTypeSize(DataTypeToEnum<DTYPE>::value) *
    ...
    OP_REQUIRES_OK_ASYNC(
        ctx,
        ctx->allocate_temp(DT_INT32, {(2 * N_ + 2) * comm->world_size() + N_},
                           host_sizes_buffer, host_alloc_attrs),
        done_);
    int32* host_input_sizes = host_sizes_buffer->flat<int32>().data();
    int32* host_output_sizes =
        host_sizes_buffer->flat<int32>().data() + N_ * comm->world_size();
    int32* host_input_total_sizes =
        host_sizes_buffer->flat<int32>().data() + 2 * N_ * comm->world_size();
    int32* host_output_total_sizes = host_sizes_buffer->flat<int32>().data() +
                                     (2 * N_ + 1) * comm->world_size();
    int32* host_n_output_sizes = host_sizes_buffer->flat<int32>().data() +
                                 (2 * N_ + 2) * comm->world_size();
    ...
    comm->stream()->LaunchUntilComputeDone(
        ...
        [n_input_sizes, n_output_sizes, host_input_sizes, host_output_sizes,
         ...
         comm, ctx, done_, this]() {
          ...
          for (int64 idx = 0; idx < N_; ++idx) {
            ctx->op_device_context()->stream()->ThenMemcpy(
                host_input_sizes + idx * comm->world_size(),
                se::DeviceMemoryBase(
                    n_input_sizes->at(idx).flat<int32>().data(),
                    n_input_sizes->at(idx).TotalBytes()),
                n_input_sizes->at(idx).TotalBytes());
            ctx->op_device_context()->stream()->ThenMemcpy(
                host_output_sizes + idx * comm->world_size(),
                se::DeviceMemoryBase(
          ...
          ctx->op_device_context()->stream()->BlockHostUntilDone();
          for (int64 rank = 0; rank < comm->world_size(); ++rank) {
            host_input_total_sizes[rank] = 0;
            host_output_total_sizes[rank] = 0;
            for (int64 idx = 0; idx < N_; ++idx) {
              host_input_total_sizes[rank] +=
                  host_input_sizes[idx * comm->world_size() + rank];
              host_output_total_sizes[rank] +=
                  host_output_sizes[idx * comm->world_size() + rank];
            }
          }
          for (int64 idx = 0; idx < N_; ++idx) {
          ...
          OP_REQUIRES_OK_ASYNC(
              ...
              call(*buffer_input, host_input_total_sizes,
                   ...
                   buffer_comm_input, buffer_comm_output, ctx, comm, this),
              done_);
          comm->stream()->BlockComputeUntilDone(ctx);
