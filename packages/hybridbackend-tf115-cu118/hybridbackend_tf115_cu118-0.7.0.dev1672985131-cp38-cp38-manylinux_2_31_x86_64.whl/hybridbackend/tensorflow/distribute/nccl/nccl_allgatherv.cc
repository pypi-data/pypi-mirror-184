...
class NcclAllgathervOp : public NcclCommAsyncOp {
 public:
  ...
  void ComputeAsyncWithComm(NcclComm* comm, OpKernelContext* ctx,
    ...
    comm->stream()->LaunchUntilComputeDone(ctx, [input, host_sizes, this, comm,
                                                 ctx, done_]() {
      ...
      OP_REQUIRES_OK_ASYNC(
          ctx,
          ctx->allocate_temp(DT_INT32, TensorShape({comm->world_size()}), sizes,
                             ctx->output_alloc_attr(0)),
          done_);
      comm->stream()->ThenWaitUntilComputeDone(ctx);
      comm->stream()->BlockHostUntilDone();

      // Collect sizes of all inputs across devices.
      VLOG(1) << comm->DebugString() << " [" << name() << "] [Allgather]";
      OP_REQUIRES_OK_ASYNC(ctx, comm->Allgather(*input_sizes, sizes), done_);
      ...
      if (is_same_size) {
        ...
        OP_REQUIRES_OK_ASYNC(ctx, comm->Allgatherv(*input, *host_sizes, output),
                             done_);
      }
