...
class EnrollServices:
    ...
    def enroll_undertake_list(params, need_pagination=True):
        ...
        enroll_obj = EnrollRecord.objects.annotate(thread_id=F("enroll__thread_id")).filter(**params).order_by(
            "-id").values()
        if not need_pagination:
            # serializer = EnrollRecordListSerializer(data=enroll_obj)
            # serializer.is_valid()
        ...
        try:
            ...
            if err:
                ...
                for v in finance_transact_data:
                    transact_id = v['transact_id']
                    v['transact_id'] = transact_id.rstrip(transact_id[-1]) + str(int(transact_id[-1]) + 2)
                    if int(transact_id[-1]) == 1:
                        v['sand_box'] = sand_box_meet
                    elif int(transact_id[-1]) == 2:
                        v['sand_box'] = sand_box_receivable
                    finance_platform_data_set, finance_platform_err = FinanceTransactService.post(v)
