...
class CrowdNoiseGenerator:
    ...
    def ui_options_changed(self):
        match self.world.uiOptions.crowd_kind:
            case const.BOT_GOAL_CAPTURE_MAP:
                driver_kind = MapCaptureDriver
            case const.BOT_GOAL_NONE:
...
class MapCaptureDriver(ExcitementDriver):
    def __init__(self, noise_generator):
        ...
        self.world.uiOptions.onChange.addListener(self.ui_options_changed, lifespan=self.lifespan)
    ...
    def ui_options_changed(self):
        if self.game_over:
            return
        if self.world.uiOptions.winning_teams or self.world.uiOptions.winning_players:
            self.game_over = True
