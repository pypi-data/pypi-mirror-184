...
def decompose_gaussian_mixtures(scan, progressPrint = False, debugplots = False, debugplotPrefix = None):
    ...
    res = {
        ...
        'Q' : _decompose_gaussian_mixture(scanx, meanQ, progressPrint = progressPrint, debugplotPrefix=decompose_gaussian_mixtures__catstrconditional(debugplotPrefix, 'I'), debugplots = debugplots),
        'A' : _decompose_gaussian_mixture(scanx, amp, progressPrint = progressPrint, debugplotPrefix=decompose_gaussian_mixtures__catstrconditional(debugplotPrefix, 'I'), debugplots = debugplots),
        'IZero' : _decompose_gaussian_mixture(scanx, meanIZero, progressPrint = progressPrint, debugplotPrefix=decompose_gaussian_mixtures__catstrconditional(debugplotPrefix, 'I'), debugplots = debugplots),
        'QZero' : _decompose_gaussian_mixture(scanx, meanQZero, progressPrint = progressPrint, debugplotPrefix=decompose_gaussian_mixtures__catstrconditional(debugplotPrefix, 'I'), debugplots = debugplots),
        'AZero' : _decompose_gaussian_mixture(scanx, ampZero, progressPrint = progressPrint, debugplotPrefix=decompose_gaussian_mixtures__catstrconditional(debugplotPrefix, 'I'), debugplots = debugplots),
        'IDiff' : _decompose_gaussian_mixture(scanx, meanIDiff, progressPrint = progressPrint, debugplotPrefix=decompose_gaussian_mixtures__catstrconditional(debugplotPrefix, 'I'), debugplots = debugplots),
        'QDiff' : _decompose_gaussian_mixture(scanx, meanQDiff, progressPrint = progressPrint, debugplotPrefix=decompose_gaussian_mixtures__catstrconditional(debugplotPrefix, 'I'), debugplots = debugplots),
        'ADiff' : _decompose_gaussian_mixture(scanx, ampDiff, progressPrint = progressPrint, debugplotPrefix=decompose_gaussian_mixtures__catstrconditional(debugplotPrefix, 'I'), debugplots = debugplots)
    }
...
def decompose_gaussian_mixtures_plotfit(scan, result):
    ...
    for iaxis, axis in enumerate(result):
        ...
        ax[iaxis][0].plot(mainaxis, sumofdiffgaussian(fit_params, mainaxis), 'r--', label = "Fit")
        ax[iaxis][0].legend()
        ax[iaxis][0].grid()
...
def _decompose_gaussian_mixture(scanx, scandata, debugplots = False, debugplotPrefix = None, progressPrint = False):
    ...
    while True:
        ...
        if len(startparams) > 0:
            ...
            indata = indata - sumofdiffgaussian(fit_params, exdata_x)
        ...
        # Start with a first initial gaussian
        fit_params = Parameters()
        ...
        out = minimize(sumofdiffgaussian, fit_params, args=(exdata_x,), kws = {'data' : indata })

        if debugplots:
            ax[1].plot(exdata_x, indata, label = "Stage input")
            ax[1].plot(exdata_x, sumofdiffgaussian(out.params, exdata_x), 'r--', label = "New seed candidate")
            ax[1].grid()
            ax[1].legend()
        ...
        try:
            out = minimize(sumofdiffgaussian, fit_params, args=(exdata_x,), kws = {'data' : exdata })
        ...
        if debugplots:
            ...
            ax[2].plot(exdata_x, sumofdiffgaussian(out.params, exdata_x), 'r--', label = f"Fitting iteration {len(startparams)}")
            ax[2].grid()
            ax[2].legend()
        
        residuum = sumofdiffgaussian(out.params, exdata_x) - exdata
        if debugplots:
            ax[3].plot(exdata_x, residuum, label = "Residuum")
            ax[3].grid()
            ax[3].legend()

        residuumremain.append(np.max(residuum) - np.min(residuum))

        if debugplots:
            ax[4].plot(residuumremain)
            ax[4].set_xlabel("Iteration")
            ax[4].set_ylabel("Residuum")
