...
def _extract_message_from_detail(detail: Any) -> str:
    if isinstance(detail, list):
        ...
        for error in errors:
            # It's possible for error to be None (see below). Return the first non-None error.
            if error != EMPTY_STRING:
                return error
        # We shouldn't get here in theory.
        return EMPTY_STRING
        ...
        # We have an empty dictionary which can happen if one of the fields of a modal has no validation errors.
        return EMPTY_STRING
    else:
        return str(detail)
