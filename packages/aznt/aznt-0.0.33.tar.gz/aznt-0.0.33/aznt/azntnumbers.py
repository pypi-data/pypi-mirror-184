...
data_rows = 29
_x = [10 ** i for i in range(1, data_rows + 1)]
...
_pnt1 = list(map(pnt, _x))
x_pi_x = [_x[i] / pi_x[i] for i in range(data_rows)]
log_x = list(map(log, _x))
approx_error = [(abs(x_pi_x[i] - log_x[i])) / x_pi_x[i] * 100 for i in range(data_rows)]


def pstats1(x=data_rows):
    """Returns statistics of prime numbers up to 10e29"""
    if x <= 0:
        print("Incorrect argument")
        return None
    headers = ["x", "\u03C0(x)", "PNT: x/logx", "x/\u03C0(x)", "logx", "(x/\u03C0(x) - logx) / x/\u03C0(x) [%]"]
    try:
        print(f"{headers[0]:^10}{headers[1]:^30}{headers[2]:^25}{headers[3]:^25}{headers[4]:^25}{headers[5]:^30}")
        for i in range(x):
            print(f"{_x[i]:<10.0e}{pi_x[i]:<30}{_pnt1[i]:<25}{x_pi_x[i]:<25}{log_x[i]:<25}{approx_error[i]:<30}")
    except IndexError:
        print(f"Function argument must be in <1; {data_rows}> range")


_pnt2 = list(map(li, _x))


def pstats2(x=data_rows):
    """Returns correlation between Li(x), Pi(x) and x/logx"""
    if x <= 0:
        print("Incorrect argument")
        return None
    headers = ["PNT: Li(x)", "\u03C0(x)", "PNT: x/logx"]
    try:
        print(f"{headers[0]:^30}{headers[1]:^30}{headers[2]:^25}")
        for i in range(x):
            print(f"{float(_pnt2[i]):<30}{pi_x[i]:<30}{_pnt1[i]:<25}")
    except IndexError:
        print(f"Function argument must be in <1; {data_rows}> range")
...
def pstats3(x=data_rows):
    """Returns statistics of Li(x) - Pi(x) prime numbers rests up to 10e29"""
    if x <= 0:
        print("Incorrect argument")
        return None
    headers = ["x", "\u03C0(x)", "PNT: Li(x)", "Li(x) - \u03C0(x)", "(Li(x) - \u03C0(x)) / Pi(x) [%]"]
    try:
        print(f"{headers[0]:^10}{headers[1]:^30}{headers[2]:^30}{headers[3]:^25}{headers[4]:^30}")
        for i in range(x):
            print(f"{_x[i]:<10.0e}{pi_x[i]:<30}{float(_pnt2[i]):<30}{approx_error_absolute[i]:<25f}{approx_error_relative[i]:<30f}")
    except IndexError:
        print(f"Function argument must be in <1; {data_rows}> range")
