...
class LMEProblem(Problem):
    ...
    def generate(groups_sizes: Optional[List[Optional[int]]] = None,
        ...
        fit_fixed_intercept : bool, default is False
            If True then the the model_name adds intercept to the set of fixed features. Intercept should not be
        ...
        true_random_effects: np.ndarray
            ...
            If None then generated according to the model_name: u_i ~ 𝒩(0, diag(𝛄)).
        ...
        return_true_model_coefficients : bool, default is True
            If True, the second return argument is a dict with true model_name coefficients: beta, gamma, random effects and
    ...
    def from_x_y(x: np.ndarray,
        ...
        if must_include_fe or must_include_re and not columns:
            raise ValueError("'columns' must be provided when 'not_regularized_fe' or 'not_regularized_re' are provided")
