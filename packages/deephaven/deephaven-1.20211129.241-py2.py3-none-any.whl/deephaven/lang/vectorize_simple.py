...
def compile_function(src, cols=None, globs=None, loc=None):
    ...
    if (is_vectorize_friendly):
        ...
        if (len(analyze.used_cols) == 0):
            ...
            if is_py38_or_higher:
                vectorized_function = numba.jit(
                    numba.core.typing.signature(nb.types.Array(return_type, 1, 'C'), (numba.types.int32)))(compiled)
            else:
                vectorized_function = numba.jit(
        ...
        if is_py38_or_higher:
            argsSig, rt = numba.core.sigutils.normalize_signature("(int32," + ",".join(argument_types) + ",)" if len(
