...
def convertToJavaArray(input, boxed=False, fallbackType=None):
    """
    ...
    * ndarrays of any other dtype (namely *complex\*, uint\*, void\*,* or custom dtypes):
    ...
    :param fallbackType:  The type to use when the array is empty, or otherwise can't be directly determined.
    ...
    if _isJavaArray(input):
        ...
        arr = makeJavaArray(input, 'unknown', convertUnknownToString=False, fallbackType=fallbackType)
        if boxed:
            return _ensureBoxedArray(arr)
        return arr
    elif isinstance(input, list) or isinstance(input, tuple):
        ndarray = numpy.asarray(input)
        return convertToJavaArray(ndarray, boxed=boxed, fallbackType=fallbackType)
...
def convertToJavaList(input, fallbackType=None):
    """
    ...
    jarray = convertToJavaArray(input, boxed=True, fallbackType=fallbackType)  # it's got to be boxed, or jpy will mangle the matching
    return jpy.get_type('java.util.Arrays').asList(jarray)


def convertToJavaArrayList(input, fallbackType=None):
    """
    ...
    javaList = convertToJavaList(input, fallbackType=fallbackType)
    return jpy.get_type('java.util.ArrayList')(javaList)


def convertToJavaHashSet(input, fallbackType=None):
    """
    ...
    javaList = convertToJavaList(input, fallbackType)
...
def makeJavaArray(data, name, convertUnknownToString=False, fallbackType=None):
    """
    ...
    * ndarrays of any other dtype (namely *complex\*, uint\*, void\*,* or custom dtypes):
    ...
    junk = _convertNdarrayToImmutableSource(data, name, convertUnknownToString=convertUnknownToString, fallbackType=fallbackType)
    if junk is None:
...
def _convertNdarrayToImmutableSource(data, name, convertUnknownToString=False, fallbackType=None):
    """
