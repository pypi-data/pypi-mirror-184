...
sales_query = """
            ...
            join "prod2-generico"."bill-items-1" a on
                f."id" = a."bill-id"
            left join public."inventory-1" b on
                a."inventory-id" = b."id"
            left join public."drugs-mis-2022-12-31" c on
                c."id" = b."drug-id"
            left join "prod2-generico"."stores" d on
                d."id"= b."store-id"
            left join "prod2-generico"."invoices-1" ii 
...
customer_returns_query = """
 SELECT
        ...
        ii."franchisee-invoice" 
    FROM
        "prod2-generico"."customer-returns-1" a
    JOIN "prod2-generico"."customer-return-items-1" b ON
        a."id" = b."return-id"
    LEFT JOIN public."inventory-1" c ON
        c."id" = b."inventory-id"
    LEFT JOIN public."drugs-mis-2022-12-31" d ON
        d."id" = c."drug-id"
    LEFT JOIN "prod2-generico"."bills-1" e ON
        e."id" = b."bill-id"
    left join "prod2-generico"."invoices-1" ii 
...
inventory_query = """
        select
            ...
            ii."franchisee-invoice" 
        from
            public."inventory-1-mis-2022-12-31" a
        left join public."drugs-mis-2022-12-31" b on
            a."drug-id" = b."id"
        left join "prod2-generico"."invoice-items-1" c on
...
purchase_from_wc_query = """
    ...
    from
        "prod2-generico"."invoices-1" a
    join
            "prod2-generico"."invoice-items-1" b on
        a."id" = b."franchisee-invoice-id"
    left join
            public."drugs-mis-2022-12-31" c on
        c."id" = b."drug-id"
    join
            "prod2-generico"."invoice-items" i on
...
from
	...
	order by
		...
		ii."franchisee-invoice"
	from
		"prod2-generico"."return-items-1" r
	left join public."inventory-1" y on
		y."id" = r."inventory-id"
	left join public."drugs-mis-2022-12-31" p on
		p."id" = y."drug-id"
	left join "prod2-generico"."debit-note-items-1" dni 
        on
		r.id = dni."item-id"
	left join "prod2-generico"."debit-notes-1" d on
	...
	left join "prod2-generico"."invoice-items-1" ii2 
            on
...
old_donotuse_zippin_return_data_query = """
        ...
        left join public."drugs-mis-2022-12-31" p on
            p."id" = y."drug-id"
        join "prod2-generico"."debit-notes-1" d on
            r."debit-note-reference" = d."id"
        join "prod2-generico"."distributors" e on
            d."dist-id" = e."id"
        join "prod2-generico"."stores" s on
...
workcell_return_data_query = """
        ...
        from
            "prod2-generico"."return-items" as r
        left join "prod2-generico"."inventory" y on
            r."inventory-id"= y."id"
        left join public."drugs-mis-2022-12-31" o on
            o."id" = y."drug-id"
        join "prod2-generico"."debit-notes" as d on
            r."debit-note-reference" = d."id"
        join "prod2-generico"."distributors" as e on
            d."dist-id" = e."id"
        join "prod2-generico"."stores" as s on
            d."store-id" = s."id"
        left join "prod2-generico"."invoices" ii 
        on y."invoice-id" = ii.id 
        left join "prod2-generico"."invoices-1" ii1 
        on ii1."invoice-reference" = ii.id 
        left join "prod2-generico"."invoice-items" ii2 
            on
        y."invoice-item-id" = ii2.id
        where
            d."dispatched-at">='{analysis_start_time}'
...
local_purchase_data_query = """
        ...
        from
            public."inventory-1" a
        join
                "prod2-generico"."invoice-items-1" b on
            a."invoice-item-id" = b."id"
        left join
               "prod2-generico"."invoices-1" c on
            a."franchisee-invoice-id" = c."id"
        left join
                public."drugs-mis-2022-12-31" d on
            a."drug-id" = d."id"
        left join
                "prod2-generico"."stores" s on
            s."id" = a."store-id"
        left join 
                "prod2-generico"."distributors" x on
...
ethical_margin_query = """
    ...
    from
        "prod2-generico"."invoice-items" a
    join "prod2-generico"."invoices" b on
        a."invoice-id" = b."id"
    join "prod2-generico"."distributors" c on
        c."id" = b."distributor-id" 
    join public."drugs-mis-2022-12-31" d on
        d."id" = a."drug-id"
    where
        c."credit-period">0
        and d."type" = 'ethical'
...
ethical_margin_fofo_query = """
  select
    ...
    join "prod2-generico"."invoices" b on
        a."invoice-id" = b."id"
    left join "prod2-generico"."invoices-1" ii on
...
home_delivery_data_query = """
    ...
    left join "prod2-generico"."bills-1" b on
        b."id" = pso."bill-id"
    left join "prod2-generico"."delivery-tracking" dt 
                        on
        dt."patient-store-order-id" = pso."id"
    left join "prod2-generico"."store-slots" ss 
                  on
        pso."slot-id" = ss.id
    where
...
other_files_inventory_at_dc_near_expiry_data_query = """
    select
        ...
        b."expiry"
    from
        "prod2-generico"."invoices" a
    join "prod2-generico"."invoice-items" b on
        a."id" = b."invoice-id"
    join public."drugs-mis-2022-12-31" c on
        b."drug-id" = c."id"
    where
        "status" = 'approved'
...
goodaid_store_returns_query = """
    ...
    left join "prod2-generico"."customer-returns-1" b on
        b."id" = a."return-id"
    left join public."inventory-1" c on
        c."id" = a."inventory-id"
    left join "prod2-generico"."stores" s on
...
goodaid_zippin_inventory_query = """
    ...
    from
        public."inventory-1-mis-2022-12-31" a
    join "prod2-generico"."invoice-items-1" c on
        a."invoice-item-id" = c."id"
    left join public."drugs-mis-2022-12-31" b on
        a."drug-id" = b."id"
    left join "prod2-generico"."stores" s on
...
goodaid_dc_inventory_query = """
    ...
    from
        ...
        from
            "prod2-generico"."inventory" a
        join "prod2-generico"."invoice-items" b on
            a."invoice-item-id" = b.id
        join public."drugs-mis-2022-12-31" dgs on
            dgs.id = a."drug-id"
        where
            "dgs"."company" = 'GOODAID'
        group by
    ...
    union
        ...
        from
            public."inventory-1-mis-2022-12-31" a
        join "prod2-generico"."invoice-items-1" b on
            a."invoice-item-id" = b.id
        join public."drugs-mis-2022-12-31" dgs on
            dgs.id = a."drug-id"
        where
    ...
    union
        ...
        from
            "prod2-generico"."return-items" a
        join "prod2-generico"."returns-to-dc" b on
            a."return-id" = b.id
        join "prod2-generico"."inventory" c on
            a."inventory-id" = c.id
        join public."drugs-mis-2022-12-31" dgs on
            dgs.id = c."drug-id"
        join "prod2-generico"."invoice-items" d on
