...
class TaskStatusHandle:
    ...
    def __str__(self):
        if self.current_status:
            return f"{self.current_status.task_name} "+tqdm.format_meter(self.current_status.progress_current or 0, self.current_status.progress_total or 0, elapsed=self.current_status.duration_sec or 0) + f" [{self.current_status.state}]" +(f" >> Current subtask: {self.current_status.current_subtask}" if self.current_status.current_subtask else "" )
        else:
    ...
    def wait_until_finished(self, polling_interval_sec:int = 15, timeout_sec:int = 60*60*6,  print_progress:bool=True):
        ...
        for _it in wait_itterator:
            if print_progress:
                print_out=str(self)
                print(print_out, end= (" "*(last_print_len-len(print_out))) +'\r', flush=True)
                last_print_len=len(print_out)
        if print_progress:
            print(str(self), end=(" "*(last_print_len-len(print_out))), flush=True)
