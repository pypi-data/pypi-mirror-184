        ...
        while True:
            ...
            if result['status'] == 'OK':
                ...
                for i in [
                    ...
                    ('INVALID_INPUT', 'Moshkeli dar darkhast pish oomad !'),
                    ...
                    if result.status == 200:
                        ...
                        if result['status'] == 'OK':
                            ...
                            for i in [
                                ...
                                ('NOT_REGISTERED', 'Moshkeli dar darkhast pish oomad !'),
                                ('TOO_REQUESTS', 'Darkhast bish az had !')
                            ]:
                                if result['status_det'] == i[0]:
                                    raise IndexError(i[1])
...
class bot:
    ...
    def get_chats_update(self):
        while True:
            ...
            if not tools.message(chats_update).message_id() in self.got_messages_update:
                if not tools.message(chats_update).chat_type() in self.filters['chat_filter'] and not tools.message(chats_update).message_type() in self.filters['message_filter']:
                    self.got_messages_update.append(tools.message(chats_update).message_id())
                    return chats_update
                else:
                    continue
            else:
    ...
    def ban_member_from_group(self, group_id, user_guid):
        return self.method(
            'banGroupMember',
            {
                'group_guid': group_id,
...
class bot_async:
    ...
    async def get_messages_info(self, chat_id, messages_id):
        return (await self.method(
            'getMessagesByID',
            {
                'object_guid': chat_id,
    ...
    async def ban_member_from_group(self, guid, user_guid):
        return await self.method(
            'banGroupMember',
            {
                'group_guid': guid,
