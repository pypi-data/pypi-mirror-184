...
PID_FILEPATH = os.path.expandvars("$HOME/.mtat-mtatweditor/mtat-mtatweditor.pid")
...
def cmd_quit(port=17310):
    try:
        ...
        logger.info("mtat-mtatweditor quit successfully")
    except requests.ConnectionError:
        logger.info("mtat-mtatweditor already stopped")
        ...
        logger.info("mtat-mtatweditor was killed")
...
def run_web(debug=False, port=17310, open_browser=False, force_quit=False):
    ...
    if version:
        if force_quit:
            logger.info(f"quit previous mtat-mtatweditor server (version: {version})")
            requests.get(base_url + "/quit")
            time.sleep(.5)
        else:
            sys.exit(f"Another mtat-mtatweditor({version}) is already running")
...
def create_shortcut():
    ...
    shortcut.SetArguments("-m mtat-mtatweditor")
...
def main():
    ...
    ap.add_argument('-p', '--port', type=int, default=17310, help='local listen port for mtat-mtatweditor')
    ...
    ap.add_argument("--quit", action="store_true", help="stop mtat-mtatweditor")
    args = ap.parse_args()
    # yapf: enable
