...
class LoginHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        is_custom_user_id = invoke_result_data.data["is_custom_user_id"] if invoke_result_data.data.__contains__("is_custom_user_id") else False
        ...
        invoke_result_data = user_base_model.save_user_by_openid(app_id, act_id, open_id, user_nick, avatar, union_id, login_ip, is_update_user_nick, is_custom_user_id)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
