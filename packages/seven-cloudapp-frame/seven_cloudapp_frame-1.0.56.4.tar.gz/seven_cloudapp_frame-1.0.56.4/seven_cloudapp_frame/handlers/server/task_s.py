...
class SaveTaskInfoHandler(ClientBaseHandler):
    ...
    def post_async(self):
        ...
        for item in task_list:
            ...
            route_url = int(item["route_url"]) if item.__contains__("route_url") else ''
            i1 = int(item["i1"]) if item.__contains__("i1") else 0
            i2 = int(item["i2"]) if item.__contains__("i2") else 0
            s1 = int(item["is_release"]) if item.__contains__("s1") else ''
            s2 = int(item["s2"]) if item.__contains__("s2") else ''
