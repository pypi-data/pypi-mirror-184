...
class AuthSys():
	def __init__(self):
		self.main_api = "https://arabshadows.ml/Apis/V1/AuthSys/index.php"
		
	def Login(self, key: str)->str:
		mac = str(get_mac_address())
		key = str(key)
		req = requests.get("{}?choice=login&key={}&mac={}".format(self.main_api,key,mac)).text
		response = json.loads(req)
		login = response["login"]
		status = response["status"]
		return response
	
	def Register(self, time: str)->str:
		req = requests.get("{}?choice=register&time={}".format(self.main_api, time)).text
		response = json.loads(req)
		access_token = response["access_token"]
		key = response["key"]
		secret_key = response["auth_secret"]
		return response
	
	def Remove(self, key: str, secret_auth: str)->str:
			req = requests.get("{}?choice=delete&key={}&auth={}".format(self.main_api,key,secret_auth)).text
			response = json.loads(req)
