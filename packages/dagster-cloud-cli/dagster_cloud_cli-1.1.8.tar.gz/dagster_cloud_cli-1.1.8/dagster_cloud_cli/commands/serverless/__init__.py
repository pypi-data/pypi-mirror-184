...
@app.command(
    name="deploy-docker",
    short_help="Add a code location from a local directory, deployed as a Docker image. "
...
@app.command(
    ...
    short_help="[Fast Deploys] Add a code location from a local directory, deployed as a Python executable. "
    "Also see 'deploy-docker' as an alternative deployment method.",
)
@dagster_cloud_options(allow_empty=True, requires_url=True)
...
def deploy_python_executable_command(
    ...
    build_in_linux_docker: bool = Option(
        False,
        help="Build the Python executable within a Linux Docker environment. "
        "This is useful if some of the dependencies require a Linux build environment and you are running on "
        "another OS.",
    ),
    **kwargs,  # pylint: disable=unused-argument
):
