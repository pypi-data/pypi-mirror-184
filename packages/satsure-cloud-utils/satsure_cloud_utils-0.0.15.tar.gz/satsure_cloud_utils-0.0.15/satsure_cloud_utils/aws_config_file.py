...
class AWSConfig:    
    def __init__(self,config_file_path:str) -> None:
        ...
        if len(config_variables["SESSION_TOKEN"]) > 0:
            command = f"""aws configure set default.region ap-south-1 --profile {self.AWS_PROFILE_NAME}; aws configure set aws_access_key_id '{config_variables["ACCESS_KEY_ID"]}' --profile {self.AWS_PROFILE_NAME}; aws configure set aws_secret_access_key '{config_variables["SECRET_ACCESS_KEY"]}' --profile {self.AWS_PROFILE_NAME}; aws configure set aws_session_token '{config_variables["SESSION_TOKEN"]}' --profile {self.AWS_PROFILE_NAME};"""
        else:
            command = f"""aws configure set default.region ap-south-1 --profile {self.AWS_PROFILE_NAME}; aws configure set aws_access_key_id '{config_variables["ACCESS_KEY_ID"]}' --profile {self.AWS_PROFILE_NAME}; aws configure set aws_secret_access_key '{config_variables["SECRET_ACCESS_KEY"]}' --profile {self.AWS_PROFILE_NAME};"""
