...
def oauth_url(
    ...
    *,
    ...
    permissions: :class:`~versacord.Permissions`
        The permissions you're requesting. If not given then you won't be requesting any
        permissions.
    guild: :class:`~versacord.abc.Snowflake`
...
def find(predicate: Callable[[T], Any], seq: Iterable[T]) -> Optional[T]:
    ...
    that meets the predicate. For example: ::
        ...
        member = versacord.utils.find(lambda m: m.name == 'Mighty', channel.guild.members)

    would find the first :class:`~versacord.Member` whose name is 'Mighty' and return it.
...
def _key_fmt(key: str) -> str:
    # Private helper for `versacord.utils.get`. Formats the attribute
...
def get(iterable: Iterable[T], **attrs: Any) -> Optional[T]:
    ...
    .. code-block:: python3
        ...
        member = versacord.utils.get(message.guild.members, name='Foo')

    Multiple attribute matching:

    .. code-block:: python3

        channel = versacord.utils.get(guild.voice_channels, name='Foo', bitrate=64000)

    Nested attribute matching:

    .. code-block:: python3

        channel = versacord.utils.get(client.get_all_channels(), guild__name='Cool', name='general')
...
def resolve_invite(invite: Union[Invite, str]) -> str:
    """
    Resolves an invite from a :class:`~versacord.Invite`, URL or code.

    Parameters
    ----------
    invite: Union[:class:`~versacord.Invite`, :class:`str`]
        The invite.
...
def resolve_template(code: Union[Template, str]) -> str:
    """
    Resolves a template code from a :class:`~versacord.Template`, URL or code.

    .. versionadded:: 1.4

    Parameters
    ----------
    code: Union[:class:`~versacord.Template`, :class:`str`]
        The code.
...
def escape_mentions(text: str) -> str:
    ...
    .. note::
        ...
        within messages, refer to the :class:`~versacord.AllowedMentions`
        class.
...
def parse_raw_mentions(text: str) -> List[int]:
    """A helper function that parses mentions from a string as an array of :class:`~versacord.User` IDs
...
def parse_raw_role_mentions(text: str) -> List[int]:
    """A helper function that parses mentions from a string as an array of :class:`~versacord.Role` IDs
...
def parse_raw_channel_mentions(text: str) -> List[int]:
    """A helper function that parses mentions from a string as an array of :class:`~versacord.abc.GuildChannel` IDs
