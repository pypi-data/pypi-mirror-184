...
# As versacord.types exist, we cannot import types
if TYPE_CHECKING:
...
class SlashCommandOption(BaseCommandOption, SlashOption, AutocompleteOptionMixin):
    ...
    def __init__(
        ...
        if typehint_origin is Literal:
            ...
            if found_type is MISSING:
                raise NotImplementedError(
                    "This behavior currently isn't handled in versacord. Please join the versacord Discord server and "
    ...
    def verify(self) -> bool:
        ...
        if self.channel_types and self.type is not ApplicationCommandOptionType.channel:
            raise ValueError(
                "channel_types can only be given when the var is typed as versacord.abc.GuildChannel"
            )
