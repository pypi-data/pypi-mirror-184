...
class Client:
    ...
    def application_id(self) -> Optional[int]:
        ...
        after :func:`~versacord.on_connect` is called.
    ...
    def application_flags(self) -> ApplicationFlags:
        """:class:`~versacord.ApplicationFlags`: The client's application flags.
    ...
    def _schedule_event(
        ...
        **kwargs: Any,
        ...
        # Schedules the task
        return asyncio.create_task(wrapped, name=f"versacord: {event_name}")
    ...
    async def on_error(self, event_method: str, *args: Any, **kwargs: Any) -> None:
        ...
        Check :func:`~versacord.on_error` for more details.
        """
    ...
    def allowed_mentions(self) -> Optional[AllowedMentions]:
        """Optional[:class:`~versacord.AllowedMentions`]: The allowed mention configuration.
    ...
    def intents(self) -> Intents:
        """:class:`~versacord.Intents`: The intents configured for this connection.
    ...
    def users(self) -> List[User]:
        """List[:class:`~versacord.User`]: Returns a list of all the users the bot can see."""
    ...
    async def change_presence(
        ...
        *,
        ...
        .. code-block:: python3
            ...
            game = versacord.Game("with the API")
            await client.change_presence(status=versacord.Status.idle, activity=game)
    ...
    async def fetch_user(self, user_id: int, /) -> User:
        ...
        Retrieves a :class:`~versacord.User` based on their ID.
        You do not have to share any guilds with the user to get this information,
        however many operations do require that you do.

        .. note::

            This method is an API call. If you have :attr:`versacord.Intents.members` and member cache enabled, consider :meth:`get_user` instead.
    ...
    async def create_dm(self, user: Snowflake) -> DMChannel:
        ...
        Parameters
        ----------
        user: :class:`~versacord.abc.Snowflake`
    ...
    def add_view(self, view: View, *, message_id: Optional[int] = None) -> None:
        """Registers a :class:`~versacord.ui.View` for persistent listening.
    ...
    def remove_view(self, view: View, message_id: Optional[int] = None) -> None:
        """Removes a :class:`~versacord.ui.View` from persistent listening.
    ...
    def add_modal(self, modal: Modal, *, user_id: Optional[int] = None) -> None:
        """Registers a :class:`~versacord.ui.Modal` for persistent listening.
        ...
        Parameters
        ----------
        modal: :class:`versacord.ui.Modal`
            The view to register for dispatching.
    ...
    def remove_modal(self, modal: Modal) -> None:
        """Removes a :class:`~versacord.ui.Modal` from persistent listening.
    ...
    async def process_application_commands(self, interaction: Interaction) -> None:
        ...
        if interaction.type is InteractionType.application_command:
            _log.debug("versacord.Client: Found an interaction command.")
            if app_cmd := self.get_application_command(int(interaction.data["id"])):
                _log.debug(
                    "versacord.Client: Calling your application command now %s", app_cmd.error_name
                )
                await app_cmd.call_from_interaction(interaction)
            elif self._lazy_load_commands:
                _log.debug(
                    "versacord.Client: Interaction command not found, attempting to lazy load."
                )
                # _log.debug(f"versacord.Client: %s", interaction.data)
                response_signature = (
                    interaction.data["name"],
                    int(interaction.data["type"]),
                    interaction.guild_id,
                )
                _log.debug("versacord.Client: %s", response_signature)
                do_deploy = False
                if app_cmd := self._connection.get_application_command_from_signature(
                    ...
                    _log.debug(
                        "versacord.Client: Basic signature matches, checking against raw payload."
                    )
                    if app_cmd.is_interaction_valid(interaction):
                        _log.debug(
                            "versacord.Client: Interaction seems to correspond to command %s, associating ID now.",
                            app_cmd.error_name,
                        )
            ...
            _log.debug("versacord.Client: Autocomplete interaction received.")
            if app_cmd := self.get_application_command(int(interaction.data["id"])):
                _log.debug("versacord.Client: Autocomplete for command %s received.", app_cmd.name)
    ...
    async def on_guild_available(self, guild: Guild) -> None:
        try:
            if self._rollout_all_guilds or self._connection.get_guild_application_commands(
                ...
                _log.debug(
                    "versacord.Client: No locally added commands explicitly registered "
            ...
            _log.warning(
                f"versacord.Client: Forbidden error for {guild.name}|{guild.id}, is the commands Oauth scope "
                f"enabled? {e}"
            )
    ...
    def parse_mentions(self, text: str) -> List[User]:
        """Parses user mentions in a string and returns a list of :class:`~versacord.User` objects.

        .. note::

            This does not include role or channel mentions. See :meth:`Guild.parse_mentions <versacord.Guild.parse_mentions>`
            for :class:`~versacord.Member` objects, :meth:`Guild.parse_role_mentions <versacord.Guild.parse_role_mentions>`
            for :class:`~versacord.Role` objects, and :meth:`Guild.parse_channel_mentions <versacord.Guild.parse_channel_mentions>`
            for :class:`~versacord.abc.GuildChannel` objects.

        .. note::

            Only cached users will be returned. To get the IDs of all users mentioned, use
            :func:`~versacord.utils.parse_raw_mentions` instead.
        ...
        Returns
        -------
        List[:class:`~versacord.User`]
            List of :class:`~versacord.User` objects that were mentioned in the string.
        """
