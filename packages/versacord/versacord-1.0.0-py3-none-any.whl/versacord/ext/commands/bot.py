...
if TYPE_CHECKING:
    ...
    import aiohttp
    from versacord.activity import BaseActivity
    from versacord.enums import Status
    from versacord.flags import MemberCacheFlags
    from versacord.mentions import AllowedMentions
    from versacord.message import Message
    from versacord.types.checks import ApplicationCheck, ApplicationHook
...
MISSING: Any = versacord.utils.MISSING
...
class MissingMessageContentIntentWarning(UserWarning):
    """Warning category raised when instantiating a :class:`~versacord.ext.commands.Bot` with a
    :attr:`~versacord.ext.commands.Bot.command_prefix` but without the :attr:`~versacord.Intents.message_content`
    intent enabled.

    This warning is not raised when the :attr:`~versacord.ext.commands.Bot.command_prefix`
    is set to an empty iterable or :func:`when_mentioned <versacord.ext.commands.when_mentioned>`.
...
class BotBase(GroupMixin):
        ...
        *,
        ...
        if (
            ...
            warnings.warn(
                ...
                "See https://docs.versacord.dev/en/stable/intents.html#what-happened-to-my-prefix-commands "
    ...
    @versacord.utils.copy_doc(versacord.Client.close)
    ...
    async def can_run(self, ctx: Context, *, call_once: bool = False) -> bool:
        ...
        return await versacord.utils.async_all(f(ctx) for f in data)  # type: ignore

    async def is_owner(self, user: versacord.User) -> bool:
        """|coro|
        Checks if a :class:`~versacord.User` or :class:`~versacord.Member` is the owner of
        this bot.
    ...
    def add_cog(self, cog: Cog, *, override: bool = False) -> None:
        ...
        if existing is not None:
            if not override:
                raise versacord.ClientException(f"Cog named {cog_name!r} already loaded")
            self.remove_cog(cog_name)

        cog = cog._inject(self)
        self.__cogs[cog_name] = cog
        # TODO: This blind call to versacord.Client is dumb.
        ...
        # this being a "blind call" to versacord.Client
        ...
        try:
            if asyncio.iscoroutinefunction(setup):
                try:
                    ...
                    https://docs.versacord.dev/en/stable/faq.html#how-do-i-make-my-setup-function-a-coroutine-and-load-it
                    """
                    )
            else:
                setup(self, **extras)
    ...
    async def get_prefix(self, message: Message) -> Union[List[str], str]:
        ...
        Parameters
        ----------
        message: :class:`versacord.Message`
        ...
        if callable(prefix):
            ret = await versacord.utils.maybe_coroutine(prefix, self, message)  # type: ignore
        ...
        if isinstance(prefix, str):
            ...
            try:
                ...
                if message.content.startswith(tuple(prefix)):
                    invoked_prefix = versacord.utils.find(view.skip_string, prefix)
                else:
                    return ctx
...
class Bot(BotBase, versacord.Client):
    """Represents a discord bot.

    This class is a subclass of :class:`versacord.Client` and as a result
    anything that you can do with a :class:`versacord.Client` you can do with
    this bot.
    ...
    command_prefix
        ...
        as its first parameter and :class:`versacord.Message` as its second
    ...
    def __init__(
        ...
        *,
        ...
        intents: versacord.Intents = versacord.Intents.default(),
...
class AutoShardedBot(BotBase, versacord.AutoShardedClient):
    """This is similar to :class:`.Bot` except that it is inherited from
    :class:`versacord.AutoShardedClient` instead.
    """
    ...
    def __init__(
        ...
        *,
        ...
        versacord.AutoShardedClient.__init__(
            self,
            max_messages=max_messages,
            connector=connector,
            proxy=proxy,
