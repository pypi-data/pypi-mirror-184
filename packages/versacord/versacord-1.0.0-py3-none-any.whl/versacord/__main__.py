...
def show_version():
    ...
    version_info = versacord.version_info
    entries.append(
        "- versacord v{0.major}.{0.minor}.{0.micro}-{0.releaselevel}".format(version_info)
    )
    if version_info.releaselevel != "final":
        pkg = pkg_resources.get_distribution("versacord")
        if pkg:
            entries.append(f"    - versacord pkg_resources: v{pkg.version}")
...
_cog_template = '''from versacord.ext import commands
import versacord
...
def parse_args():
    parser = argparse.ArgumentParser(prog="discord", description="Tools for helping with versacord")
