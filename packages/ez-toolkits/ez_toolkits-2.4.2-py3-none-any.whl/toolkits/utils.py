...
def vTrue(variable=None, type=None, checklist=None):
    '''
    检查变量类型, 以及变量是否为真
    ...
    try:
        if isinstance(variable, type):
            if checklist != None and (
                isinstance(checklist, list) or
                isinstance(checklist, tuple) or
                isinstance(checklist, set) or
                isinstance(checklist, str)
            ):
                return True if variable not in checklist else False
            else:
