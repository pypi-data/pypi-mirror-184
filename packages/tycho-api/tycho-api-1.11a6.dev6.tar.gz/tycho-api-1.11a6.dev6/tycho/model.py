...
class System:
    ...
    def generate_gitea_tokens(self):
        ...
        private_key = self.indent_keys(private_key, 8)
        public_key = self.generate_public_key(key)
        public_key = self.indent_keys(public_key, 8)
        return private_key, public_key

    def indent_keys(self, key, num_of_spaces):
        key = string.split(key, '\n')
        key = [(num_of_spaces * ' ') + string.lstrip(line) for line in key]
        key = string.join(key, '\n')
        return key
