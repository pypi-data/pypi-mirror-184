    ...
    def __repr_args__(self):
        return [
            ...
            for k, v in self._iter(
                to_dict=False, exclude_defaults=True, exclude={"ca"}
            )
        ]
...
class Trade(BaseModel):
    ...
    def __init__(
        self, contract: Contract, order: BaseOrder, status: OrderStatus
    ):
...
class ComboTrade(BaseModel):
    ...
    def __init__(
        self, contract: ComboContract, order: BaseOrder, status: ComboStatus
    ):
        super().__init__(**dict(contract=contract, order=order, status=status))
