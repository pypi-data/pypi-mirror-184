...
def train(tra_set, val_set, model, config, device):
    n_epochs = config['n_epochs']  # Maximum number of epochs
    cal_loss = nn.MSELoss(reduction="mean")
    ...
    while epoch < n_epochs:
        ...
        print(f"training data loss:  {train_loss},   validation data loss:  {valid_loss}")
        epoch += 1
    ...
    def ploss(loss_record, title=''):
        ...
        y_max = math.ceil(y_max+y_max*0.1)
        figure(figsize=(6, 4))
