...
## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    def getLst_log_pod_by_pattern_andTarget_v2(self, _sincelast, _pod, _container, _namespace, _pattern, _lst_target, **kwargs):
        ...
        try:
            ...
            print(time7.currentTime7()," CHECK idAirtable ({0}) , threadNumber ({1}) , DiffMinSecondsSilentLogger_forUpdate ({2})".format(str(idAirtable),str(threadNumber),str(DiffMinSecondsSilentLogger_forUpdate)))
            # Notes :
            # Regular Expression or sed for remove 'ANSI escape sequence' = sed -r "s/\x1b\[[^@-~]*[@-~]//g"     
