...
## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    def getLst_info_singlePods_by_podname_ns_v2(self, _podname, _ns):
        lst_ipoC = []
        try:
            ...
            lst_ipoC = self.execsshcmd(cmd_gtPoC)
        except Exception as e:
    ...
    def getLst_info_allPods_by_ns_v2(self, _ns):
        lst_ipoC = []
        try:
            ...
            raws_data = self.execsshcmd(cmd_gtPoC)
            if str(raws_data).strip()!='':
                raise 'Please check your connection and try again [Fail of function "execsshcmd"]'
            else: lst_ipoC = raws_data
        except Exception as e:
    ...
    def printlf_infHeadPodContainers(self, _dict, _writeOnFile, _fullfilename, _showPrintlf):
        ...
        try:
            if _showPrintlf == True:
                print(time7.currentTime7(), "check len lst_pod :", str(len(_dict)))
                print(time7.currentTime7())

            f = None
            if _writeOnFile == True:
                f = open(_fullfilename, 'w')     
                f.write(time7.currentTime7() +' check len lst_pod :' + str(len(_dict)));f.write('\n')   
    ...
    def printlf_infParentContainer(self, f, _writeOnFile, _showPrintlf, *args):
        try:
            if _writeOnFile == True:
                f.write(time7.currentTime7() +' '*5 + 'pod idx :' + str(args[0]));f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'pod name :' + str(args[1]));f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'pod namespace :' + str(args[2]));f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'pod containers count :' + str(len(args[3])));f.write('\n')

            if _showPrintlf == True:
                print(time7.currentTime7(),' '*5,'pod idx :', args[0])
                print(time7.currentTime7(),' '*5,'pod name :', args[1])
                print(time7.currentTime7(),' '*5,'pod namespace :', args[2])
                print(time7.currentTime7(),' '*5,'pod containers count :', len(args[3]))
        except Exception as e:
    ...
    def printlf_infAllContainer(self, f, _writeOnFile, _showPrintlf, *args):
        try:
            if _writeOnFile == True:
                f.write(time7.currentTime7() +'-');f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container name :' + args[0]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container restart :' + args[1]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container started :' + args[2]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container stateStartedAt :' + args[3]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container exitCode :' + args[4]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container stateReason :' + args[5]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container lastStateReason :' + args[6]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container lastStateStartedAt :' + args[7]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container lastStateFinishedAt :' + args[8]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container STATUS :' + args[9]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container REASON :' + args[10]);f.write('\n')
                f.write(time7.currentTime7() +' '*5 + 'container AGE :' + args[11]);f.write('\n')

            if _showPrintlf == True:
                print(time7.currentTime7(),'-')
                print(time7.currentTime7(),' '*5,'container name :', args[0])
                print(time7.currentTime7(),' '*5,'container restart :', args[1])
                print(time7.currentTime7(),' '*5,'container started :', args[2])
                print(time7.currentTime7(),' '*5,'container stateStartedAt :', args[3])
                print(time7.currentTime7(),' '*5,'container exitCode :', args[4])
                print(time7.currentTime7(),' '*5,'container stateReason :', args[5])
                print(time7.currentTime7(),' '*5,'container lastStateReason :', args[6])
                print(time7.currentTime7(),' '*5,'container lastStateStartedAt :', args[7])
                print(time7.currentTime7(),' '*5,'container lastStateFinishedAt :', args[8])
                print(time7.currentTime7(),' '*5,'container STATUS :', args[9])
                print(time7.currentTime7(),' '*5,'container REASON :', args[10])
                print(time7.currentTime7(),' '*5,'container AGE :', args[11])
        except Exception as e:
    ...
    def printlf_infFooterPodContainers(self, f, _writeOnFile, _showPrintlf):
        try:
            if _writeOnFile == True:
                f.write(time7.currentTime7() +'- '*42);f.write('\n')
            if _showPrintlf == True:
                print(time7.currentTime7(),'- '*42)
        except Exception as e:
            ...
            if _strcmd.strip():
                if len(target_str) !=0:
                    for target in target_str:
                        ...
                        if threadNumber!= '' and str(idAirtable).strip() != '' and int(DiffMinSecondsSilentLogger_forUpdate) != 0:
