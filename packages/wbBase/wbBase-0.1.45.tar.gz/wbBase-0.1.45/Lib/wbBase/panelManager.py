...
class PanelManager(aui.AuiManager):
    def __init__(self, managed_wnd=None, flags=aui.AUI_MGR_DEFAULT):
        self.ManagedWindow: ApplicationWindow
        self.ArtProvider: aui.AuiDockArt
        self._menu: Optional[wx.Menu] = None
        self._toolbar: Optional[aui.AuiToolBar] = None
        self._documentNotebook: Optional[DocumentNotebook] = None
        self._workspace = {}
        self._panel_icons = {}
        self._workspaceCurrent = None
        self._tempPerspective = None
        self.mnu_workspace = None
    ...
    def config(self) -> wx.ConfigBase:
        cfg = self.ManagedWindow.config
        cfg.SetPath("/Window/Panels/")
        return cfg

    @property
    def plugins(self):
        pluginManager = self.ManagedWindow.pluginManager
        if pluginManager:
    ...
    def getPaneByCaption(self, caption) -> aui.AuiPaneInfo:
        for pane in self.AllPanes:
    ...
    def _restoreTempPerspective(self) -> None:
        if self._tempPerspective is not None:
    ...
    def on_PANE_BUTTON(self, event):
        if (
            event.Button == aui.AUI_BUTTON_MAXIMIZE_RESTORE
            and self._tempPerspective is None
        ):
    ...
    def on_tool_fullscreen(self, event):
    ...
    def on_tool_expandCenterPanel(self, event):
        pane = self.centerPane
        if pane and pane.IsOk():
            if event.IsChecked():
                self._tempPerspective = self.SavePerspective()
    ...
    def on_menu_panel(self, event: wx.MenuEvent):
    ...
    def on_menu_workspace_save(self, event: wx.MenuEvent):
    ...
    def on_menu_workspace_select(self, event):
    ...
    def on_menu_tooltip(self, event: wx.MenuEvent):
