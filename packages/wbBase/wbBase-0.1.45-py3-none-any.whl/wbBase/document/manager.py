...
from . import (
    DOC_NEW,
    DOC_NO_VIEW,
    DOC_OPEN_ONCE,  # DEFAULT_DOCMAN_FLAGS,
    DOC_SILENT,
    FileNameFromPath,
    dbg,
)
...
class DocumentManager(wx.EvtHandler):
    ...
    @property
    def plugins(self):
        pluginManager = self.TopLevelParent.pluginManager
        if pluginManager:
    ...
    def lastActiveView(self) -> Optional[View]:
        """
        The last active view. This is used in the SDI framework where dialogs can be mistaken for a view
    ...
    @maxDocsOpen.setter
    def maxDocsOpen(self, maxDocsOpen):
    ...
    def CreateDocument(self, path, flags=0):
        ...
        if path and flags & DOC_SILENT:
            ...
            temp, path = self.SelectDocumentPath(self.visibleTemplates, path, flags)
                ...
                if document.path and os.path.normcase(
                    ...
                    if not document.modificationDateCorrect:
                        ...
                        if res == wx.YES:
                            ...
                            wx.LogStatus(f"Document reloaded: {newDoc.printableName}")
                            return newDoc
                    ...
                    if firstView and firstView.frame and not flags & DOC_NO_VIEW:
                        ...
                        if (
                            hasattr(firstView.frame, "IsIconized")
                            and firstView.frame.IsIconized()
                        ):  # Not in wxWindows code but useful nonetheless
                            firstView.frame.Iconize(False)
                    dbg(
        ...
        if temp:
            newDoc = temp.CreateDocument(path, flags)
            if newDoc:
                if isinstance(newDoc, (list, tuple)):
                    for _newDoc in newDoc:
                        wx.LogStatus(f"Document opened: {_newDoc.printableName}")
                        self.log.info("document opened: %s", _newDoc.path)
                else:
                    newDoc.template = temp
    ...
    def openDocuments(self, filenames):
    ...
    def SelectDocumentType(self, templates):
        """
    ...
    def FindTemplateForPath(self, path):
        ...
        if generic:
            if len(generic) == 1:
                ...
                temp = self.SelectDocumentType(generic)
                if temp:
                    return temp
        return None

    def FindTemplateByType(self, templateType):
        for template in self._templates:
            if isinstance(template, templateType):
                return template

    def FindTemplateByDocumentTypeName(self, documentTypeName):
        for template in self._templates:
            if template.documentTypeName == documentTypeName:
                return template

    def FindTemplateByDescription(self, description):
        for template in self._templates:
    ...
    def testForExternalChanges(self, testAll=False):
        if testAll:
    ...
    def CloseDocument(self, doc: Document, force=True):
        """
    ...
    def _setupLogging(self):
        ...
        if self.app.about:
            ...
            formatter = logging.Formatter(
                f"%(asctime)s\t{self.app.AppName}\t%(message)s", datefmt="%Y-%m-%d %H:%M:%S"
            )
    ...
    def on_file_open(self, event):
        """
        Creates new documents and reads in the selected files.
        """
    ...
    def on_file_save(self, event):
        ...
        self.currentDocument.Save()
        # event.Skip()
        dbg("DocumentManager.on_file_save() -> done", indent=0)

    def on_file_saveall(self, event):
    ...
    def on_file_closeall(self, event):
        """
    ...
    def on_update_file_saveas(self, event):
        event.Enable(self.currentDocument is not None)

    def on_update_file_close(self, event):
        event.Enable(self.currentDocument is not None)

    def on_update_file_closeall(self, event):
        event.Enable(self.documentCount > 0)

    def on_update_file_revert(self, event):
        enable = False
        doc = self.currentDocument
        if (
            doc
    ...
    def on_update_file_ext_canges(self, event):
        doc = self.currentDocument
