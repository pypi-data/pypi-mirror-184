...
class DocumentTemplate:
    ...
    @property
    def documentType(self) -> Document:
        """
    ...
    # public methods
    # -----------------------------------------------------------------------------
    def GetViewType(self, viewTypeName) -> Optional[View]:
        for viewType in self._viewTypes:
            if viewType.__name__ == viewTypeName:
                return viewType


    def CreateView(self, doc:Document, flags:int, viewType=None) -> View:
        """
        ...
        """
        dbg(f'DocumentTemplate.CreateView(doc="{doc}", flags={flags})', indent=1)
        result = None
        ...
        try:
            ...
            view.Destroy()
        dbg(f"DocumentTemplate.CreateView() done -> {result}", indent=0)
        return result

    def CreateDocument(self, path, flags, **kwds) -> Document:
        """
        ...
        """
        dbg(f'DocumentTemplate.CreateDocument(path="{path}", flags={flags})', indent=1)
        result = None
        doc:Document = self._docType()
        doc.path = path
        doc.template = self
        ...
        if doc.OnCreate(path, flags, **kwds):
            ...
            if doc in self.documentManager.documents:
                doc.DeleteAllViews()
        dbg(f"DocumentTemplate.CreateDocument() returns {result}", indent=0)
        return result
    ...
    def AddViewType(self, viewType:View):
        assert issubclass(viewType, View)
