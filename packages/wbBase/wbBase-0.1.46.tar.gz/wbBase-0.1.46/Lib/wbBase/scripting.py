...
def execfile(filePath, global_vars=None, local_vars=None) -> None:
...
class MacroMenuItem(wx.MenuItem):
    def __init__(self, parentMenu, path, handler):
        ...
        self.SetBitmap(
            wx.ArtProvider.GetBitmap("wxART_PYTHON_FILE", wx.ART_MENU, (16, 16))
        )
        self.path = path
...
class MacroMenu(wx.Menu):
    def __init__(self, handler=None, folderList=None):
        ...
        if folderList:
            ...
            self.menuItem_update.SetBitmap(
                wx.ArtProvider.GetBitmap("RELOAD", wx.ART_MENU, (16, 16))
            )
    ...
    def addMacroTree(self, macroTree, menu:wx.Menu):
    ...
    def on_MenuSelection(self, event):
        menuItem = self.FindItemById(event.Id)
