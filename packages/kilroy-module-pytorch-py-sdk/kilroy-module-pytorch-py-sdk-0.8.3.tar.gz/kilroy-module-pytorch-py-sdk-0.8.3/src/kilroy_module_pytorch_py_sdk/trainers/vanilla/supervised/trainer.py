...
from kilroy_module_pytorch_py_sdk.trainers.vanilla.supervised.parameters import (
    OptimizerParameter,
    SchedulerParameter,
    MethodParameter,
    RegularizationsParameter,
)
from kilroy_module_pytorch_py_sdk.trainers.vanilla.supervised.params import (
    OptimizerParams,
    ModelParams,
    ModelsParams,
    Params,
...
from kilroy_module_pytorch_py_sdk.trainers.vanilla.supervised.state import (
    State,
    ModelsState,
    OptimizerState,
    SchedulerState,
...
class SupervisedTrainer(SupervisedTrainerBase):
    ...
    async def fit(
        ...
        async def __increment_epoch() -> int:
            async with self.state.write_lock() as state:
                state.models.policy.epoch += 1
                return state.models.policy.epoch

        controls = TrainingControls(
            model=policy, step=__step, increment_epoch=__increment_epoch
        )

        await method.fit(controls, baseline, data, regularizations or [])
