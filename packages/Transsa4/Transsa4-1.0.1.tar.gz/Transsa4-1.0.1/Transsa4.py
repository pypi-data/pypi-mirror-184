...
def InsertColumns(D): #(D es la tabla) 
    D=D.astype({'nro_dormitorios': 'float64','nro_banos':'float64','nro_estacionamientos':'float64','sup_util':'float64',
...
def datosduplicados(tabla,T): #(tabla es la tabla, T es True entonces se muestra la cantidad de datos eliminados)
    ...
    tabla2 = tabla.drop_duplicates(subset=['nro_dormitorios','nro_banos','nro_estacionamientos','longitud','latitud',
                                          'sup_util','sup_total'])
...
def Muestra(df1,df2,cve):
    ...
    N=df1.shape[0]
    n2=int(N/10)
    df3=df1.sort_values(by="valor_uf", ascending= True)
    Muestras=[]
    for i in range(0,2):
        a1=df3.iloc[0:n2,:]
        b1=df3.iloc[n2:2*n2,:]
