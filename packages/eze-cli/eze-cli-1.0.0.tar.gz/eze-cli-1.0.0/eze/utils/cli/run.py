...
async def run_async_cli_command(
    ...
    config: dict
        ...
        [optional] __THROW_ERROR_ON_STDERR bool
        [optional] __FORCED_DOCKER_SOURCE_FOLDER str modify folder to mount to docker
    ...
    forced_source_folder: str = py_.get(config, "__FORCED_DOCKER_SOURCE_FOLDER", False)
    ...
    try:
        ...
        source_folder: str = get_current_working_folder(forced_source_folder)
        docker_config = create_docker_config(cli_config, source_folder)
        if not docker_config:
            raise
        # run docker if present
        command_list = build_cli_command(docker_config, config)
