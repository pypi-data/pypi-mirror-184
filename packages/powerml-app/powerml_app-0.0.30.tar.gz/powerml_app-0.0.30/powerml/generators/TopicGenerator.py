...
class TopicGenerator(Generator):
    ...
    def __init__(self, gold_topics=unblocked_human_unprompted_1gram_topics):
        super().__init__([lemmatizer.lemmatize(topic.lower()) for topic in gold_topics])
    ...
    def coverage_generator(self, messages, return_metrics=True):
        ...
        generated_topics = [lemmatizer.lemmatize(topic.lower()) for topic in model.predict()]
        ...
        if rare_topics:
            ...
            num_generate = ceil((1 - coverage) * len(messages) / len(rare_topics))
            print(f'num generate: {num_generate}')
            for topic in rare_topics:
                modifier = f'include the topic \'{topic}\''
                generated_messages.extend(self._generate_modified(messages, modifier, num_generate))
        if return_metrics:
