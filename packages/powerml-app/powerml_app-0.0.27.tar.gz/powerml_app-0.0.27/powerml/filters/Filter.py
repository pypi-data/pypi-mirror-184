...
from powerml.utils.constants import PARALLEL_REQUEST_LIMIT, DELAY_IN_SECONDS
...
class Filter():
    ...
    def __init__(self, model, prompt):
        self.model = model
        self.prompt = prompt
        self.filters = dict(self.general_filters, Invalid=lambda data: self.__filter_invalid(data, self.model, self.prompt))
    ...
    def __fuzzy_is_valid(self, data, model, prompt): # NOTE: could add reference data for this
        valid_examples = []
        for i in range(0, len(data), PARALLEL_REQUEST_LIMIT):
            curr_data = data[i:i+PARALLEL_REQUEST_LIMIT]
            prompt_prepend = f'Is this a valid {prompt}:\n\n'
    ...
    def __filter_invalid(self, data, model, prompt):
        print('Start Filtering Invalid Data:', datetime.now())
        filtered_data = self.__fuzzy_is_valid(data, model, prompt)
