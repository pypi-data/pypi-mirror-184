...
class EditGui(object):
	def __init__(self, frame, dataBase=None, screens=None, \
		...
		self._inputTags = inputFrame.TextField(tagsSubPanel, width=30, \
								text='Tags: ', packing=tk.LEFT, state=tk.DISABLED)
	...
	def _saveSong(self, event=None): # pylint: disable=unused-argument
		if self._dataBase and self._printedElement.etype == 'song':
			...
			self._printedElement.tags = self._inputTags.get(1.0, tk.END) # pylint: disable=no-member
			try:
