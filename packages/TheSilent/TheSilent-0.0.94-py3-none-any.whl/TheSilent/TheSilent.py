...
def xss_scanner(url, secure = True, my_file = " "):
    ...
    for links in my_result:
        ...
        try:
            result = web_session.get(links, verify = False, headers = user_agent, cookies = mal_cookie, timeout = (5, 30)).text
            
            if mal_script in result:
                print(red + "true: " + links + " (cookie)")
                my_list.append(links + " (cookie)")
