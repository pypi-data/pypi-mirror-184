...
class XTController(rpyc.Service):
    ...
    def bg_queue_worker(self):
        ...
        while True:
            ...
            node_elapsed_mins = (time.time() - self.started)/60
            if self.mrc_context.max_minutes and (node_elapsed_mins > self.mrc_context.max_minutes):
                console.print("** Error: node elapsed_mins ({:.2f}) exceeded job's max_minutes ({:.2f})".format( \
                    node_elapsed_mins, self.mrc_context.max_minutes))
