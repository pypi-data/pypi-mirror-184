...
class Utils:
    ...
    @staticmethod
    def generate_params(trial: Any, params: dict) -> dict:
        """Generate trial features for an optuna study

        Args:
            trial (Any): Optuna trial object
            params (dict): Parameters dict. Each key that contains a
                suggest_type field may be added to args variables dict
    ...
    @staticmethod
    def generate_objective(params=None) -> Any:
        """Generate an optuna objective function

        Args:
            params (_type_, optional): Optuna dict of parameters. Defaults to
                None.

        Returns:
            Any: Internal objective function
        """
        def objective(trial: Any,  # pylint: disable=too-many-arguments
                      estimator: Any,
