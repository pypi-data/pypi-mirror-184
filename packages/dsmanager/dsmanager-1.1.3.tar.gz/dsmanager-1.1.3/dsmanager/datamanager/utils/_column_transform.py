...
def onehot_encode(df_: pd.DataFrame, column: str, **kwargs: Any) -> pd.DataFrame:
    ...
    return (
        df_
        .assign(**pd.get_dummies(df_[column], prefix=column, **kwargs))
    )
...
def column_spliter(df_: pd.DataFrame, column: str, columns: List[str] = None) -> pd.DataFrame:
    ...
    if columns is None:
        return (
            df_
            .assign(
                **pd.DataFrame(df_[column].to_list())
                .add_prefix(column)
            )
        )
    return (
        df_
        .assign(
            **pd.DataFrame(df_[column].to_list(), columns=columns)
            .add_prefix(column)
        )
    )
