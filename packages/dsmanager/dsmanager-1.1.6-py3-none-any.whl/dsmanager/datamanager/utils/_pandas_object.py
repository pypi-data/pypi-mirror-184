...
def pipe_sklearn(  # pylint: disable=too-many-arguments
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        pre_func (Any, optional): Filter function to apply on original df_. Defaults
            to lambda df__ : df__.
        output (List[str], optional): List of output columns expected. Defaults to
            None.
    ...
    if indices is None:
        indices = df_.index
    elif callable(indices):
        indices = indices(df_).index
    ...
    for x_param in ["X", "Xt", "data"]:
        if x_param in kwargs:
            ...
            kwargs[x_param] = df_.loc[indices][var_x]

        if x_param in signature and not var_x:
            kwargs[x_param] = df_.loc[indices]

    if "y" in kwargs:
        var_y = kwargs["y"]
        kwargs["y"] = df_.loc[indices][var_y]
        if len(var_y) == 1:
            kwargs["y"] = df_.loc[indices][var_y].values.ravel()

    res = func(**kwargs)

    if isinstance(res, np.ndarray) and len(indices) == res.shape[0]:
        if output is None:
            try:
                output = func.__self__.get_feature_names_out() or []
            except (AttributeError, ValueError) as _:
                output = []

            output = [
                func_name + "_" + str(e)
                for e in range(res.shape[-1] if res.ndim > 1 else 1)
            ]
