...
def test_find_histogram_single_frame_volume(tmp_path):
    ...
    assert find_histogram(volume=volume, scan=EDFTomoScan(scan=str(tmp_path)),) == DataUrl(
        file_path=histogram_file,
        data_path="entry",
        scheme="silx",
    )

    assert find_histogram(volume=volume, scan=HDF5TomoScan(scan=str(tmp_path), entry="entry"),) == DataUrl(
        file_path=histogram_file,
        data_path="entry",
        scheme="silx",
    )
