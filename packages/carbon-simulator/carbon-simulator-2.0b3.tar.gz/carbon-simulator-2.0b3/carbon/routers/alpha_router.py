...
class AlphaRouter(BaseRouter):
    ...
    def match_by_target(
        ...
        results.sort_values(
            by=["hypothetical_output_amts", "indexes"],
            ascending=[False, True],
            inplace=True,
        )

        results.loc[:, "sliding_index"] = [
            np.array(win.values.tolist(), dtype=object)
            for win in results.indexes.rolling(
                threshold_orders, min_periods=threshold_orders
            )
        ]
        results.loc[
            :, "sliding_available_value"
        ] = results.ordered_associated_liquidity.rolling(
            threshold_orders, min_periods=threshold_orders
        ).sum()
        ...
        top_n_threshold_orders = [
            results.sliding_index[i]
            for i in results.index
            if results.sliding_available_value[i] >= abs(x)
        ][0]
    ...
    def match_by_src(
        ...
        # print(x)
        hypothetical_output_amts = {
            i: self.amt_by_src(dx=x, subject=i) for i in self.indexes
        }
        ...
        results.loc[:, "sliding_available_value"] = results.available_value.rolling(
            threshold_orders, min_periods=threshold_orders
        ).sum()
        ...
        # Constrain the exact router to just the top n threshold orders and match
        self.exact_router.orders = self.orders
        self.use_positions_matchlevel = (
            top_n_threshold_orders  # top_n(threshold_orders)
        )
        return self.match(
            x=x,
            is_by_target=is_by_target,
