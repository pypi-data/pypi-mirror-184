	...
	def query(
		...
		**kwargs
		...
		if(not isinstance(_query, list)):
			if(or_queries:= _query.pop("$or", None)):  # top level $or query
				if(not _query):
					queries = or_queries
		...
		class MultiCollectionQueryResult:
			...
			def __next__(self):
				...
				if(self.results_returned % MONGO_WARN_MAX_RESULTS_RATE == 0):
					...
					time_elapsed = cur_timestamp - self._timestamp
					if(self.results_returned > MONGO_WARN_MAX_RESULTS_RATE * time_elapsed):
						LOG_WARN(
