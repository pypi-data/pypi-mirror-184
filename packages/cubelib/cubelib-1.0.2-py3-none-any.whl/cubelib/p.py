...
class NotImplementedPacket:
    ...
    Parameters
    hex_id
        Hexadecimal id of packet as string
    bound
        Bound of the packet
    ver_name
...
class Night:
    ...
    def destroy(cls, buff: bytes):
        ...
        for name in an:
            ...
            if isinstance(TYPE, Optional):
                if getattr(self, TYPE.field_name) != TYPE.excepted_val:
                    continue
                TYPE = TYPE.type
            try:
        ...
        try:
            ...
            raise DecoderException(f'Packet [{cls.__name__}] field over, but packet buffer not empty yet! Unexpected len: {len(unex)} Data: {unex}')
        except BufferExhaustedException:
            pass
    ...
    def _build_into_packet(self) -> Packet:
        ...
        return Packet(self._id(), payload, None, False)
    ...
    def __init__(self, *args):
        ...
        if len(an) != len(args):
            v = f'{self.__class__.__name__}(' + ', '.join(an) + ')'
