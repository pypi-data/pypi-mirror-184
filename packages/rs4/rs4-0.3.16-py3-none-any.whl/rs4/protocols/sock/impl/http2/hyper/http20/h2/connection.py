...
class H2Connection(object):
    ...
    def _begin_new_stream(self, stream_id, allowed_ids):
        ...
        if outbound:
            self.highest_outbound_stream_id = max (stream_id, self.highest_outbound_stream_id)
        else:
            self.highest_inbound_stream_id = max (stream_id, self.highest_inbound_stream_id)
