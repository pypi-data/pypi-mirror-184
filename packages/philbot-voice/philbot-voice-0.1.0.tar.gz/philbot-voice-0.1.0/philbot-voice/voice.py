...
class Context:
    ...
    def __stream(self, url, ssrc, secret_key, ip, port):
        ...
        while frame:
            ...
            # package = create_voice_package(sequence, ssrc, secret_box, bytes(frame[0].contents))
            # DEBUG
            package_1 = create_voice_package(sequence, ssrc, secret_box, bytes(frame[0].contents)[:UDP_MAX_PAYLOAD//2])
            sequence += 1
            package_2 = create_voice_package(sequence, ssrc, secret_box, bytes(frame[0].contents)[UDP_MAX_PAYLOAD//2:])
            # DEBUG
            if (len(package_1) > UDP_MAX_PAYLOAD):
                raise RuntimeError('Package too big for UDP')
            with self.lock:
                if (my_socket != self.socket):
                    return
                # my_socket.sendto(package, (ip, port))
                my_socket.sendto(package_1, (ip, port))
                my_socket.sendto(package_2, (ip, port))
            sequence += 1
            frame = file.get_buffer()
        ...
        with self.lock:
            ...
            match payload['op']:
                ...
                case 2:
                    ...
                    my_port = PORT
                    print('VOICE GATEWAY opening UDP connection')
