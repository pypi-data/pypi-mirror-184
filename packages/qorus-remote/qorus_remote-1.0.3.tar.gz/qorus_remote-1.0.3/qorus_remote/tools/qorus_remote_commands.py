                ...
                with open(ofile) as of:
                    ...
                    if 'api-manager' in doc \
                        ...
                        and 'value' in doc['api-manager']['provider-options']['schema']:
                            path = os.path.join(os.path.dirname(ofile),
                                doc['api-manager']['provider-options']['schema']['value'])
                            rfiles.append(path)
...
def oload_add_files(ofiles: set) -> set:
...
def oload_upload_files(files: set, directory: str = '') -> str:
    if not files:
        return directory
...
def oload_handle(args):
    ...
    toUpload: set = oload_add_files(set(ofiles))
    # XXX DEBUG DELETEME
    print("toUpload: {}".format(toUpload))
    directory: str = oload_upload_files(toUpload)

    if globals['cmd']['cmd'] == 'oload':
        ofiles = oload_remove_dir_from_files(ofiles)

    # XXX DEBUG DELETEME
    print("ofiles: {}".format(ofiles))
