...
#     except ValueError:
#         pass
def getValueAllowed_special(kwargs, _nameOfKwargs, _typeOfKwargs, _defaultValue = None):
    value = _defaultValue; bAllowParams = False
    print('check type :',str(type(kwargs.get(str(_nameOfKwargs)))), ' == ', _typeOfKwargs)
    if str(_nameOfKwargs).strip() in kwargs:
        if f"'{str(_typeOfKwargs)}'" in str(type(kwargs.get(str(_nameOfKwargs)))):
            print("YES type is float")
                ...
                if (not '_argMarkSilentLogger' in kwargs) and (not '_argDiffMinSecondsSilentLogger_forUpdate' in kwargs):
                    ...
                    if bparam == True:
                        if MarkSilentLogger == True:
                            ...
                            DiffMinSecondsSilentLogger_forUpdate, bparam = getValueAllowed_special(kwargs, '_argDiffMinSecondsSilentLogger_forUpdate', mpl.variable_type.float.value, 2.0)
                            if bparam == True:
