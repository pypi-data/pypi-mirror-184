...
def logger(_time=cT7(), *args, **kwargs):   
    ...
    try:
        ...
        idAirtable = ''; threadNumber = const_thread.number_1.value; threadNumber, \
                ...
                if str(idAirtable).strip()!='':
                    ...
                    if bparam == True:
                        ...
                        if MarkSilentLogger==True:
                            diffRlogger = float(diffRange_logger(cT7(),_argThreadNumber = int(threadNumber)))
                            DiffMinSecondsSilentLogger_forUpdate, \
                            bparam = mpl.kwargs().getValueAllowed(kwargs,'_argDiffMinSecondsSilentLogger_forUpdate',mpl.variable_type.float, 2.9)
                            if bparam == True:
