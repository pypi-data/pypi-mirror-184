from typing import List, Optional, Union

import terrascript.core as core


@core.schema
class AwsLambda(core.Schema):

    function_arn: Union[str, core.StringOut] = core.attr(str)

    function_payload: Optional[Union[str, core.StringOut]] = core.attr(str, default=None)

    def __init__(
        self,
        *,
        function_arn: Union[str, core.StringOut],
        function_payload: Optional[Union[str, core.StringOut]] = None,
    ):
        super().__init__(
            args=AwsLambda.Args(
                function_arn=function_arn,
                function_payload=function_payload,
            ),
        )

    @core.schema_args
    class Args(core.SchemaArgs):
        function_arn: Union[str, core.StringOut] = core.arg()

        function_payload: Optional[Union[str, core.StringOut]] = core.arg(default=None)


@core.schema
class ContentTransformation(core.Schema):

    aws_lambda: AwsLambda = core.attr(AwsLambda)

    def __init__(
        self,
        *,
        aws_lambda: AwsLambda,
    ):
        super().__init__(
            args=ContentTransformation.Args(
                aws_lambda=aws_lambda,
            ),
        )

    @core.schema_args
    class Args(core.SchemaArgs):
        aws_lambda: AwsLambda = core.arg()


@core.schema
class TransformationConfiguration(core.Schema):

    actions: Union[List[str], core.ArrayOut[core.StringOut]] = core.attr(str, kind=core.Kind.array)

    content_transformation: ContentTransformation = core.attr(ContentTransformation)

    def __init__(
        self,
        *,
        actions: Union[List[str], core.ArrayOut[core.StringOut]],
        content_transformation: ContentTransformation,
    ):
        super().__init__(
            args=TransformationConfiguration.Args(
                actions=actions,
                content_transformation=content_transformation,
            ),
        )

    @core.schema_args
    class Args(core.SchemaArgs):
        actions: Union[List[str], core.ArrayOut[core.StringOut]] = core.arg()

        content_transformation: ContentTransformation = core.arg()


@core.schema
class Configuration(core.Schema):

    allowed_features: Optional[Union[List[str], core.ArrayOut[core.StringOut]]] = core.attr(
        str, default=None, kind=core.Kind.array
    )

    cloud_watch_metrics_enabled: Optional[Union[bool, core.BoolOut]] = core.attr(bool, default=None)

    supporting_access_point: Union[str, core.StringOut] = core.attr(str)

    transformation_configuration: Union[
        List[TransformationConfiguration], core.ArrayOut[TransformationConfiguration]
    ] = core.attr(TransformationConfiguration, kind=core.Kind.array)

    def __init__(
        self,
        *,
        supporting_access_point: Union[str, core.StringOut],
        transformation_configuration: Union[
            List[TransformationConfiguration], core.ArrayOut[TransformationConfiguration]
        ],
        allowed_features: Optional[Union[List[str], core.ArrayOut[core.StringOut]]] = None,
        cloud_watch_metrics_enabled: Optional[Union[bool, core.BoolOut]] = None,
    ):
        super().__init__(
            args=Configuration.Args(
                supporting_access_point=supporting_access_point,
                transformation_configuration=transformation_configuration,
                allowed_features=allowed_features,
                cloud_watch_metrics_enabled=cloud_watch_metrics_enabled,
            ),
        )

    @core.schema_args
    class Args(core.SchemaArgs):
        allowed_features: Optional[Union[List[str], core.ArrayOut[core.StringOut]]] = core.arg(
            default=None
        )

        cloud_watch_metrics_enabled: Optional[Union[bool, core.BoolOut]] = core.arg(default=None)

        supporting_access_point: Union[str, core.StringOut] = core.arg()

        transformation_configuration: Union[
            List[TransformationConfiguration], core.ArrayOut[TransformationConfiguration]
        ] = core.arg()


@core.resource(type="aws_s3control_object_lambda_access_point", namespace="aws_s3control")
class ObjectLambdaAccessPoint(core.Resource):

    account_id: Optional[Union[str, core.StringOut]] = core.attr(str, default=None, computed=True)

    arn: Union[str, core.StringOut] = core.attr(str, computed=True)

    configuration: Configuration = core.attr(Configuration)

    id: Union[str, core.StringOut] = core.attr(str, computed=True)

    name: Union[str, core.StringOut] = core.attr(str)

    def __init__(
        self,
        resource_name: str,
        *,
        configuration: Configuration,
        name: Union[str, core.StringOut],
        account_id: Optional[Union[str, core.StringOut]] = None,
        depends_on: Optional[Union[List[str], core.ArrayOut[core.StringOut]]] = None,
        provider: Optional[Union[str, core.StringOut]] = None,
        lifecycle: Optional[core.Lifecycle] = None,
    ):
        super().__init__(
            name=resource_name,
            args=ObjectLambdaAccessPoint.Args(
                configuration=configuration,
                name=name,
                account_id=account_id,
                depends_on=depends_on,
                provider=provider,
                lifecycle=lifecycle,
            ),
        )

    @core.schema_args
    class Args(core.Resource.Args):
        account_id: Optional[Union[str, core.StringOut]] = core.arg(default=None)

        configuration: Configuration = core.arg()

        name: Union[str, core.StringOut] = core.arg()
