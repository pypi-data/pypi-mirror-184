...
class RedisCache(MutMap):
    ...
    def __init__(self, cache=None, ttl=600):
        self._cache = redis.Redis(host=os.environ.get('REDIS_HOST', 'localhost'), db=os.environ.get('REDIS_DB', 2))
        self._ttl = ttl
...
class PrefixedRedisCache(RedisCache):
    ...
    def __init__(self, prefix: str = "", ttl=600):
        super().__init__(redis.Redis(host=os.environ.get('REDIS_HOST', 'localhost'), db=os.environ.get('REDIS_DB', 2)), ttl)
        self._prefix = prefix
