...
class Tree(object):
    ...
    def deepest_label(self):
        """
        Find the deepest labeled = 1 nodes and return names
        """
        return self.deepest_label_helper(self.root,[])
    
    def deepest_label_helper(self, node, names):
        """
        Find the deepest labeled = 1 nodes and return names
        """
        if node.labeled == 1 and node != self.root:
            if node.children == []:
                ...
                for child in node.children:
                    if child.labeled == 1:
                        pos_cnt += 1
                    names = self.deepest_label_helper(child, names)
                if pos_cnt == 0:
                    names.append(node.concept_name)
        return names
