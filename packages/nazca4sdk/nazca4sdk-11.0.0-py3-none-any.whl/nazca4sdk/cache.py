...
class Cache:
    ...
    def load(self):
        ...
        """
        try:
            response = requests.get(f'{self._base_url}/api/Config/modulesDefinitions', verify=False)
    ...
    def read_historical_variable(self, variable_historical_info: VariableHistoricalInfo):
        ...
        try:
            exist_vars = self._check_if_exist(variable_historical_info.module_name,
            ...
            if not exist_vars:
                ...
                return None
            variables_grouped = self._check_variables(variable_historical_info.module_name,
                                                      variable_historical_info.variable_names)
            params = [
            ...
            for variable_type in variables_grouped:
                for variable_name in variable_type[1]:
                    params.append((f'groupedVariables[{variable_type[0]}]', variable_name))
    ...
    def variable_over_day(self, module_name, variable_names, start_date, end_date):
        ...
        try:
            exist_vars = self._check_if_exist(module_name, variable_names)
            if not exist_vars:
                print(f"Module {module_name} or {variable_names} not exist")
                return None
            variables_grouped = self._check_variables(module_name, variable_names)
    ...
    def _check_variables(self, module, variable):
        grouped_variables = list(map(lambda x: [x, self._types[module][x]], variable))
    ...
    def _check_if_exist(self, module: str, variables: list):
        """
    ...
    def read_variables_stats(self, module: str, variables: str, start_date: str, end_date: str):
        exist_vars = self._check_if_exist(module, [variables])
        if not exist_vars:
            print(f"Module {module} or  variables {variables} not exist")
            return None
        variables_grouped = self._check_variables(module, variables)
