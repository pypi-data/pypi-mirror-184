...
class VariableForPrediction(BaseModel):
    """Class to verify the number of variable names
    ...
    @validator('variable_names')
    def variable_names_verify_number(cls, variable_names):
        """Validator to check the number of input variable names
        ...
        if len(variable_names) != 1:
            raise ValueError('Number of variables has to be equal 1')

        return variable_names


class ForecastForPrediction(BaseModel):
    """Class to verify the forecast time amount ant unit to make future dataframe to for prediction
...
class PredictionTool(BaseModel):
    """Class to verify the prediction tool
