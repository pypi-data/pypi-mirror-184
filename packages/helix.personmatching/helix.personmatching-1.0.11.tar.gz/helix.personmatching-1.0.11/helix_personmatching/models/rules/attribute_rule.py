...
class AttributeRule(Rule):
    ...
    def calculate_string_match(
        attribute: str, val_source: Optional[str], val_target: Optional[str]
    ) -> float:
        val_source_cleansed: str = str(val_source).strip().lower()
        val_target_cleansed: str = str(val_target).strip().lower()

        if attribute == Attribute.ADDRESS_POSTAL_CODE:
            if (
                (val_source_cleansed != val_target_cleansed)
                and (val_source_cleansed not in val_target_cleansed)
                and (val_target_cleansed not in val_source_cleansed)
            ):
                return 0.0

        score_for_attribute = fuzz.ratio(val_source_cleansed, val_target_cleansed)
