...
class EndpointTomorrowIO(EndpointWeather):
    ...
    def make_conn_str(
        ...
        conn_str = f"/v4/timelines?location={lat}%2C{lon}&fields={value_name}\
...
class EndpointWeatherAPI(EndpointWeather):
    ...
    def parse_request_cc(self, response: Dict[str, Any]) -> int:
        """
        The parse_request_cc function takes in a response from the DarkSky API and
    ...
    def make_conn_str(self, lon: float, lat: float, key: str) -> str:
        """
        ...
        conn_str = f"/v1/current.json?key={key}&q={lon},{lat}&aqi=no"
        return conn_str
