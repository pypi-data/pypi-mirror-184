...
class FullHouse(BaseHand):
    ...
    def __hand_outs__(self) -> List[Card]:
        ...
        if len(self.ranks_pair) >= 2:
            # In this case, we already have 2 pair, so need to iterate over each pair we have and get the remaining suits
            rank_outs = self.ranks_pair
        elif len(self.ranks_triple) >= 1 and len(self.ranks_single) >= 1:
            # In this case, we already have a triple. So a pair with any of the ranks_single will complete the full house
