...
def get_exp_from_gemfile(input_file):
    ...
    if 'x' in df.columns and 'y' in df.columns:
        if 'MIDCounts' in df.columns:
            ...
            count = df['MIDCounts'][1:].astype(np.int32)
        elif 'MIDCount' in df.columns:
            x = df['x'][1:].astype(np.int32)
            y = df['y'][1:].astype(np.int32)
            count = df['MIDCount'][1:].astype(np.int32)
...
def save_exp_heat_map_by_binsize(input_file, output_png, bin_size, scale=2, dpi=72):
    if input_file.endswith('.gem') or input_file.endswith('.gz'):
        ...
        if isHeaderInFile:
            if binSize == bin_size:
                fileError, d_x, d_y, d_cnt = get_exp_from_gemfile(input_file)
                if fileError:
                    print("The file cannot be recognized!")
                    return False
            else:
        ...
        if f'/wholeExp/bin{bin_size}' in h5f:
            d_x = h5f['geneExp'][f'bin{bin_size}']['expression']['x']
            d_y = h5f['geneExp'][f'bin{bin_size}']['expression']['y']
            d_cnt = h5f['geneExp'][f'bin{bin_size}']['expression']['count']
        else:
            if f'geneExp/bin{bin_size}' in h5f:
                d_x = h5f['geneExp'][f'bin{bin_size}']['expression']['x']
                d_y = h5f['geneExp'][f'bin{bin_size}']['expression']['y']
                d_cnt = h5f['geneExp'][f'bin{bin_size}']['expression']['count']
            else:
                if 'geneExp/bin1' in h5f:
                    bgef = BgefR(input_file, bin_size, 4)
                    exp = bgef.get_reduce_expression()
                    d_x = exp[:, 0]
                    d_y = exp[:, 1]
