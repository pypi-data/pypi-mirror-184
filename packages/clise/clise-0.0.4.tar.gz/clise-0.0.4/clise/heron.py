...
def diagnostic(cfg):
	...
	if type(show_what).__name__ == 'NoneType':	
...
def execute(cfg):
	...
	if cfg.get('-dump','') != '':
		try:
			...
			print("cannot dump cfg since serializing cfg failed")
		# if not rabbit: return True
	...
	if 'time' in show_what: start_time=datetime.now()
...
def go(cfgs=None, befores=None, afters=None, ovw=None, cli=sys.argv[1:], level=-1):
	global taskkey, mt_refer, maxtask, show_what
