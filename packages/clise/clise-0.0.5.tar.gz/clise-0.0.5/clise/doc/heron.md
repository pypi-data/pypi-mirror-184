...
executions of the routine: ver 0.0.5 by Jaesub Hong (jhong@cfa.harvard.edu)
...
access to modify. In the latter, you may have to write your own wrapper to
bypass that. CLISe takes a different approach.

CLISe enables executions of any function as a command script only with input
parameters either in a command line or with an input file (in 
an extended JSON format). CLISe dynamically make a decorator for the called
...
the function name in future runs. For more complex operations, CLISe allows 
sequential calls of multiple functions, and provide a simple mechanics to
...
limited to leave out the programming part: i.e., "rabbit.py" provides the auto
task repetition, but it doesn't provide a range based for-loop functionality,
which belongs to the programming.

The input parameter files contain the name of the routine to call:
...
In principle, all the contents in the JSON files can be fed as a long string in
...
For clise (heron.py)
      ...
      -init       bool  true  initialize class or define class parameters
                  or dict
...
- How to define multiple calls in serial using JSON
      ...
      JSON file without "-main" (or "-load") is considered as additional options
      for other calls.
      ...
      esp. for repeating a set of multiple calls
            {
                  ...
                  ...
                  "-block" : "rabbit:release", // this indicates the starting block
            },
            {
            ...
            },
            {
                  ...
                  // now conditioning parameter setting
                  "-id==ID1" : {
                        "par1":  ...
                  },
                  "-id!=ID2" : {
                  ...
                  "-block" : "rabbit:capture", // this indicates the ending block for task gen
            },
            {
                  "-main" : "moduleB.routineB", //  an independent run 
                  ...
