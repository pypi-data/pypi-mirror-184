...
AUTO_GEN = 'rabbit.multiply_auto_by_file'
MANU_GEN = 'rabbit.multiply_manual_by_id'
...
def group_cli(cli=sys.argv[1:]):
	...
	if len(stackfile) == 0:
		return [orphan+mergefile+merge_cli+stack_cli]
	...
	for par in jsonpar:
		...
		if bool(mat):
			...
			for each in cfg_file:
				if '-main' not in each and '-load' not in each and \
					each.get('-rabbit', False) >= False:
					if len(base) == 0: before=merge(before, each)
					else:			 after =merge(after , each)
				else:
					base = base + [each]
...
def load_cli(cli=sys.argv[1:]):
	cli_sets = group_cli(cli)
	cfgs    = []
	befores = []
	afters  = []
	itask= 0
	for cli in cli_sets:
		base, before, after = analyze_cli(cli) # a bit of redundancy
		for task in base:
