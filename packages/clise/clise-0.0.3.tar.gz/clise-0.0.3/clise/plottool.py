...
def plot1d_core(x, y, attr=None, xscale='linear',  
		xlow=None, xhigh=None, ylow=None, yhigh=None, xerr=None, yerr=None,
...
#				image=np.transpose(hdul[hdu].data)
		...
		xdata = xdexpr(xexpr, xdata, ydata, vars=vars)
		ydata = ydexpr(yexpr, xdata, ydata, vars=vars)
		xerrdata = errexpr(xeexpr, xerrdata, xdata, ydata, vars=vars)
		yerrdata = errexpr(yeexpr, yerrdata, xdata, ydata, vars=vars)
		zdata = zdexpr(zexpr, xdata, ydata, vars=vars)
...
def plot1d(xdata=None, ydata=None, data=None, 
	...
	def show(ion=True):
		...
		pid, ax2, ay2 = plot1d_core(xdata, ydata, attr=attr, xscale=xscale,  
			...
			color=color, label=None, alpha=alpha,
			marker=marker, linestyle=linestyle, linewidth=linewidth,
...
#			set_rcParams(rcParams, verbose=verbose)
	...
	def mplot1d(self, outfile=None, xr=None, yr=None, xscale=None, yscale=None, margin=None):
		...
		if outfile != None: self.outfile=outfile
		if xr      != None: xrf=xr
		if yr      != None: yrf=yr
		if xscale  != None: self.xscale=xscale
		if yscale  != None: self.yscale=yscale
		...
		def show(ion=True):
			...
			for key in self.xdata:
				...
				pid, ax2, ay2 = plot1d_core(x, y, attr=self.attr[key], 
					xlow=self.xlowdata[key], xhigh=self.xhighdata[key], ylow=self.ylowdata[key], 
					yhigh=self.yhighdata[key], xerr=self.xerrdata[key], yerr=self.yerrdata[key],
			...
			self.wrap(plt, ax, xr=xrf, yr=yrf, ax2=ax2, ay2=ay2)
			return ax, ax2, ay2
