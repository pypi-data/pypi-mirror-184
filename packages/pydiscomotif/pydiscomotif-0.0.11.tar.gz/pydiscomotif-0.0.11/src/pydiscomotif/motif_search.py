...
def run_subgraph_monomorphism(motif_MST: nx.Graph, map_of_candidate_PDBs_and_their_residue_pairs: Dict[str, Set[Tuple[str, str]]]) -> Dict[str, List[nx.Graph]]:
    """
    ...
    """
    monomorphism_checked_motifs: Dict[str, List[nx.Graph]] = defaultdict(list) # Each PDB can have multiple motifs (ie: homodimers) -> List[nx.Graph]
    for PDB_ID, pairs_of_residues in map_of_candidate_PDBs_and_their_residue_pairs.items():
        ...
        for residue_mapping_dict in graph_matcher.subgraph_monomorphisms_iter():
            ...
            monomorphism_checked_motifs[PDB_ID].append(similar_motif_graph)
...
def filter_out_PDBs_with_unconnected_residue_pairs(
    ...
    submited_futures = {
        concurrent_executor.submit(run_subgraph_monomorphism, motif_MST, map_of_candidate_PDBs_and_their_residue_pairs):motif_MST
