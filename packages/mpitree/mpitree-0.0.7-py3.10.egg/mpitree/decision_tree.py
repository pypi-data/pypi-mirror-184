"""Module containing the `DecisionTreeClassifier` and `DecisionTreeRegressor` implementations."""
...
class DecisionTreeClassifier(DecisionTreeEstimator):
    ...
    def _make_tree(self, X, y, *, parent=None, branch=None, depth=0):
        ...
        for *d, level in levels:
            best_node += self._make_tree(*d, parent=y, branch=level, depth=depth + 1)
        return best_node
...
class DecisionTreeRegressor(DecisionTreeEstimator):
    ...
    def _find_weighted_variance(self, X, y, d):
        ...
        if is_numeric_dtype(X[d]):
            ...
            weight = np.unique(X[d], return_counts=True)[1] / len(X)
            metric = [
                self._metric(X.loc[X[d] == t], y.loc[X[d] == t])
    ...
    def _make_tree(self, X, y, *, branch=None, depth=0):
        ...
        for *d, level in levels:
            best_node += self._make_tree(*d, branch=level, depth=depth + 1)
        return best_node
