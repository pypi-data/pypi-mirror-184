"""Module containing the `ParallelDecisionTreeClassifier` implementation."""
        ...
        if size == 1:
            for *d, level in levels:
                best_node += self._make_tree(
                    *d, comm=comm, parent=y, branch=level, depth=depth + 1
                )
        else:
            ...
            sub_tree = comm.allgather(
                {
                    level: self._make_tree(
                        *d, comm=group, parent=y, branch=level, depth=depth + 1
                    )
                }
            )
