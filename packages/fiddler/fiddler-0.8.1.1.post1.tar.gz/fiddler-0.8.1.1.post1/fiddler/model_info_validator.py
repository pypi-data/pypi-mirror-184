...
class ModelInfoValidator:
    ...
    def validate_ranking(self):
        ...
        if self.model_info.outputs[0].data_type != DataType.FLOAT:
            raise ValueError('model output must be of type FLOAT')
        if self.model_info.targets[0].data_type in [DataType.STRING, DataType.CATEGORY]:
            raise ValueError(
                'Target cannot be of type string or category. Please review dataset schema to be type int or float.'
            )
