import snv_feast.helpers.helpers as helpers
import yaml
import os
import pandas as pd
...
# Example python signature_snv_selection.py --strain Bacteroides_uniformis_57318 --min_reads 5 --start_index 1 --end_index 200 --config_file_path /Users/leahbriscoe/Documents/FEASTX/snv-feast/configs/config.yaml
# signature_snvs("Bacteroides_uniformis_57318", 5, 1, 200, "/Users/leahbriscoe/Documents/FEASTX/snv-feast/configs/config.yaml")


def signature_snvs(strain, min_reads, start_index, end_index, config_file_path):
    ...
    for fam in unique_family_ids:
        ...
        final_family_private_allsnps_counts.to_csv(
            output_dir + fam_file_string + "_counts.csv.bz2",
            compression="bz2",
            header=False,
            index=False,
        )
    ...
    all_fam_LRs.to_csv(
        output_dir + "Summary_Stats_" + file_string + "_LRs.csv", sep=","
    )

    all_fam_presents = pd.concat(all_fam_present, axis=1)
    all_fam_presents.to_csv(
        output_dir + "Summary_Stats_" + file_string + "_fam_presence.csv", sep=","
    )
