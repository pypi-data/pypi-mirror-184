    ...
    def _watch_recv(self):
        ...
        while True:
            ...
            with self._received_added:
                if res:
                    res = json.loads(res)
                    self._emit("traffic:*", "received", res)
                    self._emit("traffic:received", res)
                    # add response to _received and notify waiting
                    if "id" in res:
                        self._received[res["id"]] = res
                        self._emit("message", res)
                        self._emit_handle(res["id"], "traffic:*", "received", res)
                        self._emit_handle(res["id"], "traffic:received", res)
                        self._received_added.notify_all()
                    else:
                        self._emit(
                            "notification" if "params" in res else "message", res
                        )
                    if "change" in res:
                        self._emit_handles(res["change"], "changed")
                    if "close" in res:
                        self._emit_handles(res["close"], "closed")
