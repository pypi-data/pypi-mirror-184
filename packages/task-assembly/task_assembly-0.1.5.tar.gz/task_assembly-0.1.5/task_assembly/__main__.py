...
def create_task_type(name):
    task_type_id = client.create_task_type(name)
    print(f"Created task type ID: {task_type_id}")


def create_task_definition(name, task_type_id):
    definition = client.create_task_definition(name, task_type_id)
...
def update_task_definition(definition_file):
...
def get_task_definition(id, definition_file=None):
    definition = client.get_task_definition(id)
...
def create_task(definition_file, assignments, sandbox, values, max_assignments, quals):
    definition = read_definition(definition_file)
    params = {
...
def get_task(task_id, include_assignments=False):
    response = client.get_task(task_id, include_assignments)
    print(json.dumps(response, indent=4))


def stop_task(task_id):
    client.expire_task(task_id)


def redrive_task(task_id, extend):
    response = client.redrive_task(task_id, extend=extend)
    print(json.dumps(response, indent=4))
...
def submit_batch(definition_file, name, input_file, s3_uri_prefix, sandbox=False, assignments=None):
    definition = read_definition(definition_file)
    name = name.replace(" ", "_")
    extension = os.path.splitext(input_file)[1][1:].lower()
    delimiter = delimiter_map.get(extension)
    if not delimiter:
        raise Exception("Input file must have an extension of csv or tsv")
...
def get_batch_results(batch_id, output_file):
    response = client.get_batch(batch_id)
    try:
        ...
        if len(results) == 0:
            ...
            if ext == "jsonl":
                ...
                raise Exception("Input file must have an extension of json, jsonl, csv, or tsv")
...
def list_workers(definition_file, output_file):
    definition = read_definition(definition_file)
...
def list_batches(definition_file):
    definition = read_definition(definition_file)
    response = client.list_batches()


def redrive_scoring(definition_file):
    definition = read_definition(definition_file)
    client.redrive_scoring(definition["DefinitionId"])


def stop_batch(batch_id):
    client.expire_batch(batch_id)


def redrive_batch(batch_id, extend):
    client.redrive_batch(batch_id, extend)


def resolve_batch(batch_id, extend):
    client.resolve_batch(batch_id, extend)
...
def main():
    ...
    ct_parser.set_defaults(func=create_task)
    ...
    gt_parser.set_defaults(func=get_task)

    st_parser = subparsers.add_parser("stop_task")
    st_parser.add_argument("task_id")
    st_parser.set_defaults(func=stop_task)
    ...
    rt_parser.set_defaults(func=redrive_task)
    ...
    lw_parser.set_defaults(func=list_workers)
    ...
    rb_parser.set_defaults(func=redrive_batch)
    ...
    rsb_parser.set_defaults(func=resolve_batch)
