...
class TgWrap():
    ...
    def construct_command(self, command, debug, exclude_external_dependencies,
        ...
        commands = {
            ...
            'destroy': '{base_command} {command} --terragrunt-non-interactive --terragrunt-no-auto-approve {ignore_deps} {debug_level} {working_dir} {parallelism} {terragrunt_args}',
        }
                    ...
                    if dry_run:
                        ...
                        try:
                            ...
                            if rc.returncode != 0 or 'error' in messages.lower():
                                raise Exception(
                                    f'An error situation detected while processing the terragrunt dependencies graph'
                                    )
    ...
    def run_sync(
        ...
        if not self.is_installed('rsync'):
            ...
            self.printer.header("Will be deploying:")
    ...
    def git_deploy(
        ...
        try:
            ...
            if target_stage == 'global' and source_stage != target_stage:
                self.printer.error(f'You are trying to copy source stage {source_stage} to a globa stage!')
                sys.exit(1)
