"""Utilities for shortening path in the TensorWrap API.
Exporting a function or a class :
Use tf_export decorator. For e.g.:
...
from tensorwrap.util import decorator
from tensorwrap.util import inspect

# ESTIMATOR_API_NAME = 'estimator'
NN_API_NAME = 'nn'
TENSORWRAP_API_NAME = 'tensorwrap'

# List of subpackage names used by TensorWrap components. Have to check that
# TensorWrap core repo does not export any symbols under these names.
...
def get_canonical_name_for_symbol(
    symbol, api_name=TENSORWRAP_API_NAME,
  ...
  _, undecorated_symbol = decorator.unwrap(symbol)
