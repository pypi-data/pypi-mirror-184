...
def decdeg2degminsec(arr:_np.ndarray):
    """Expects input array of decimal degrees. Could be used with 2D arrays: decdeg2degminsec(arr[:,:2])"""
    sign = _np.sign(arr)
    arr_u = _np.abs(arr)
    d = arr_u.astype(int)
    arr_u_remainder = arr_u - d
    m = (arr_u_remainder * 60).astype(int)
    s = (arr_u_remainder - m/60)*3600
    return sign*d,m,s.round(1)